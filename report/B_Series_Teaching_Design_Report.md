...
> **建立日期**：2026-03-30
> **版本**：v1.5（2026-04-01 更新：Rounds 43–44 — 記憶遮蔽、翻牌動畫、步驟卡、計算過程顯示）
> **前次版本**：v1.4（2026-04-01，Round 44 — B1 倒數計時、B2 主題導引、B3 里程碑語音、B4 思路卡、B6 計算動畫）
> **前次版本**：v1.3（2026-03-31 更新：Round 42 — B5 分組佈局、B3 4選項、B1 逐項語音）
...
## 十五、Rounds 43–44 教學強化：記憶、思維與引導（2026-04-01）

### 15.1 設計主軸：認知負荷梯度（Cognitive Load Gradient）

Rounds 43–44 的核心設計哲學是**在困難模式引入更高認知負荷，同時在簡單/普通模式提供更豐富的引導支架**。透過這兩個方向的同步強化，B 系列形成完整的「負荷梯度」設計：

| 難度 | 設計策略 | 認知負荷 |
|------|---------|---------|
| 簡單（easy） | 逐項語音、步驟卡、圖示動畫導引 | 低→中（分散化）|
| 普通（normal）| 自動提示、進度視覺化、貪婪演算法提示 | 中 |
| 困難（hard） | 記憶遮蔽、翻牌揭示、倒數計時 | 高（接近真實情境）|

### 15.2 Round 43 — 困難模式感知遮蔽系統

| 單元 | 遮蔽對象 | 揭示方式 | 教學意義 |
|------|---------|---------|---------|
| B2 | 事件 icon + 名稱（blur 6px）| `👁️ 查看` toggle | 訓練工作記憶，「先聽後算」策略 |
| B3 | 每週存款金額（blur + ???）| `👁️ 提示` 一次性揭示 | 強化「聽語音→記數字→計算」認知鏈 |
| B5 | 選購商品價格（??? 元）| 首次點擊翻牌揭示（0.33s scaleX）| 模擬真實店面翻看價格標籤行為 |

**設計一致性**：三個單元共用「模糊遮蔽→主動揭示」的 UX pattern，降低學習曲線（學生學會一個單元後能快速適應其他單元）。

B4 困難模式增加三商店獎台語音播報，視覺（獎台動畫）與聽覺（語音）雙軌確認排名，對應「多感官記憶鞏固」教學原則。

### 15.3 Round 44 — 思維引導與情境計時

#### B1 行程卡倒數計時器

```
easy=30s / normal=20s / hard=15s
≤5s → b1-rt-urgent 紅色脈動（緊迫感）
時間到 → error 音+語音+費用明細
答對 → 立即清除計時器
```

**教學原理**：適度時間壓力（Time Pressure）能提高注意力與計算效率，但不應造成焦慮。30/20/15 秒梯度設計確保各難度均有足夠思考時間，紅色脈動給予視覺預警而非突然失敗。
...
**教學原理**：引導學生建立「兩步比較」的思維程序（A→B→結論），而非直覺跳到答案。只在 easy 模式顯示，不干擾 normal/hard 的自主推理。

#### B2 類別圖示動畫導引

每道新題開始時顯示主題圖示（如 📚 學校週記），2s 後淡出。功能類似「情境切換信號」，幫助學生快速調整到正確情境框架，降低題目理解的認知搜尋成本。
...
- 50%：「存了一半了，真棒！」
- 75%：「快到了，差一點點！」
- 100%：「達標了！{itemName}可以買了！」

**教學原理**：**具體進度語音**比通用表揚更能強化目標感知（Goal Progress Effect），讓學生感受到「距離目標越來越近」的動機增強效應。
...
付了 X 元
商品 Y 元
找零 Z 元  ← 逐行進場，每行 400ms
```

**教學原理**：顯示計算過程（而非只顯示結果）呼應「程序性知識」（Procedural Knowledge）教學——學生不只記住答案，還能看到「如何計算找零」的步驟。

### 15.4 Round 41–42 — 視覺差異化與動畫回饋

| 單元 | 功能 | 教學設計哲學 |
|------|------|------------|
| B2 easy | 收入綠漸層/支出紅漸層卡片 | 顏色編碼降低類型辨識負荷（Perceptual Load Theory）|
| B3 quiz | 4選項 + 結構化干擾 | 干擾項對應最常見計算錯誤，診斷學習盲點 |
| B4 | 動態價格浮動 ±10~20% | 訓練「每次題目不同」的真實比價能力，防止背答案 |
| B5 | 必買/選購雙色分組 | 視覺區分「必要支出」vs「彈性選擇」的預算概念 |
| B1 | 硬幣逐一動畫（280ms stagger）| 慢速呈現強化「面額→累計」的感知連結 |
| B1 | 費用逐項語音（950ms/項）| C2 pattern：「一邊聽一邊看」降低認知負荷 |

### 15.5 B 系列 vs A/C/F 系列跨系列移植總表（Rounds 24–44）

| 功能類型 | 參照來源 | B 系列實作 |
|---------|---------|----------|
| 逐項語音播報 | C2 逐一計數 | B1 `_speakItemsOneByOne` |
| 困難模式遮蔽 | C1 音色識別 | B2 `b2-memory-mode`、B3 `b3-weekly-hidden`、B5 `b5-price-hidden` |
| 翻牌揭示動畫 | A1 coinFirst 亮燈 | B5 `b5-flip-reveal` |
| 步驟引導卡 | A4 輔助點擊流程卡 | B4 `_showThinkingSteps` |
| 計算過程顯示 | C6 找零算式 | B6 `_animateChangeCalc` |
| 倒數計時器 | A5 ATM 計時 | B1 `_startRouteTimer` |
| 里程碑語音 | F2 里程碑音效 | B3 `celebSpeeches` |
| 主題情境導引 | A3 場景切換 | B2 `_showThemeGuide` |
| 獎台動畫 | F4 排序動畫 | B4 `_showPodiumAnimation` |
| 顏色面額標籤 | C4 面額高亮 | B6 `b6-hint-badge` + `--bc` |

---

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-04-01（v1.5）*
