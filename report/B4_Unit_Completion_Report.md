...
> **更新日期**：2026-04-03（互動豐富化：①困難模式差額輸入區提示按鈕新增吉祥物圖示（`b4-diff-hint-btn` 外包裹 inline-flex div）；②錯誤語音統一：選最便宜「不對喔，請再比較看看」、三商店排序「不對喔，請看看正確的排序，再試一次！」、差額輸入「不對喔，差額是X元，請再試一次」；搜尋關鍵字：`不對喔，請再比較看看`、`不對喔，差額是`）
> **更新日期**：2026-04-04（三商店 easy/normal 模式 3次錯誤自動提示：`selectErrorCount >= 3` 觸發 → 錯誤語音改「不對喔，X才是最便宜的，請再選看看」（直接透露答案）+ 高亮最便宜商品（`b4-select-hint` class，`b4AutoHint` 脈動 2.4s）；搜尋關鍵字：`tripleAutoHint`、`tripleHintSpeech`）
> **更新日期**：2026-04-04（差額階段漸進提示：①新增 `quiz.diffErrorCount` 追蹤差額錯誤次數（`nextQuestion()` 同步重置）；②第1~2次錯誤：普通/困難均改為給方向提示（`算多了/算少了，再想想看`）不直接透露答案；③第3次以上：顯示算式提示 `_showDiffFormulaHint()` + 說出正確答案；④三商店差額段同步套用；⑤困難模式提示按鈕語音改為完整語句「A店X元，B店Y元，兩者差多少元？」；搜尋關鍵字：`diffErrorCount`、`revealAnswer`、`revealTriple`、`兩者差多少元`）
