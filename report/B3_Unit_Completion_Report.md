...
> **更新日期**：2026-04-03（月曆模式放置區重構 + 普通/困難提示按鈕系統）
...
## 十二、月曆模式放置區重構 + 提示按鈕系統（2026-04-03）

### 12.1 放置區個別金錢圖示

| 修改前 | 修改後 |
|--------|--------|
| 依面額分組顯示 `×N`（`b3-nplaced-row` + `b3-nplaced-count`） | 每枚金錢各顯獨立圖示（`b3-nplaced-item`） |
| 紙鈔/硬幣尺寸差距不明顯 | 紙鈔 `68px`、硬幣 `44px`（1.55倍差距） |

**搜尋關鍵字**：`b3-nplaced-item`、`imgSize.*68px`

### 12.2 困難模式語音行為修正

- **放置時不播總額**：`_handleNormalDrop` 加 `difficulty !== 'hard'` 守衛，困難模式放置時不播報目前總額
- **確認後才提示錯誤**：困難模式移除超額即時拒絕（允許任意面額放入），僅在按「確認」後由 `_confirmNormalDeposit` 判斷
- **錯誤語音統一**：太多 → 「不對喔，你存的錢太多，請再試一次」；太少 → 「不對喔，你存的錢太少，請再試一次」

**搜尋關鍵字**：`difficulty.*hard.*Speech`、`不對喔.*存的錢太多`
...
- 靜態 HTML：`b3-daily-header-row`（flex space-between）內嵌 `b3-daily-hint-wrap`（吉祥物 28px + 提示鈕）
- 按鈕預設隱藏（`display:none`），`_updateNormalDailyCard()` 在普通/困難模式開始新一天時顯示
- `_completeDragSession()` 存款完成後隱藏提示鈕

**搜尋關鍵字**：`b3-daily-hint-btn`、`b3-daily-hint-wrap`

### 12.4 普通模式：Ghost Slots 引導提示

**流程**：

1. 按「💡 提示」→ 貪婪演算法分解目標金額 → 建立 `c.drag.hintSlots[]`（`{ denom, filled }`）
2. 若已有放置項目 → 播退回動畫（`b3-nplaced-return`，220ms）→ 清除後顯示 ghost slots
3. 播語音「可以存入 N 個 X 元、M 個 Y 元」
4. Ghost slot 渲染：`b3-nplaced-ghost-slot`，使用獨立動畫 `b3NDenomInGhost`（結尾 `opacity:0.35`，避免被 `b3NDenomIn` 的 `opacity:1` 覆蓋）
5. 每個 slot 加 `data-hint-idx` 索引
6. 拖曳正確面額 → `_handleNormalDrop` 找到第一個 `denom` 匹配且未填的 slot → 直接操作 `[data-hint-idx]` DOM 移除 `b3-nplaced-ghost-slot` class → CSS `transition: opacity 0.25s ease` 平滑從淡化變正常（**不重繪整體，避免閃爍**）
7. 拖曳錯誤面額（無匹配 slot）→ 播錯誤音，拒絕
8. 清除按鈕 → 所有 `hintSlots` 重置 `filled: false`
9. 再按「💡 隱藏」→ 清除 `hintSlots`，放置區恢復空白

**搜尋關鍵字**：`hintSlots`、`data-hint-idx`、`b3NDenomInGhost`、`b3-nplaced-ghost-slot`

### 12.5 困難模式：提示彈窗

- `_showHardModeHintModal()`：貪婪分解目標金額 → 彈窗顯示面額圖示（紙鈔 62px / 硬幣 48px）
- 同步播語音「今天要存 X 元，可以用 N 個 X 元…」
- 彈窗結構：`b3-hint-modal-overlay`（遮罩）> `b3-hint-modal`（卡片）> 標題 + 圖示 + 關閉鈕
- 點「✕ 關閉」或點遮罩區域關閉；同一時間只存在一個彈窗（`id="b3-hard-hint-modal"` 防重複）

**搜尋關鍵字**：`_showHardModeHintModal`、`b3-hint-modal-overlay`、`b3-hard-hint-modal`

### 12.6 提示語音格式修正

| 修改前 | 修改後 |
|--------|--------|
| 數量 1 時省略（「5元」） | 永遠顯示「1個5元」 |
| 分隔符「、」 | 分隔符「，」 |
| 普通：「可以存入2個10元、5元」 | 普通：「可以存入2個10元，1個5元」 |
| 困難：「今天要存25元，可以用2個10元、5元」 | 困難：「今天要存25元，可以用2個10元，1個5元」 |

**搜尋關鍵字**：`可以存入.*個.*元`、`cnt}個${d}元`
