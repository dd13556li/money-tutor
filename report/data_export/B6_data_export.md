...
> 匯出日期：2026-04-29（更新：新增三市場預算對照表 / 各市場任務預算列表 / 章節重編 / 第九節改為魔法商品系統）
...
**傳統市場預算範圍**：簡單 **60–100 元**、普通 **110–180 元**、困難 **180–260 元**
...
| 難度 | 各關預算 | 範圍 |
|------|---------|------|
| 簡單 | 80, 80, 80, 100, 100, 100, 100, 100 | **80–100 元** |
| 普通 | 130, 150, 160, 170, 180, 190, 200, 200 | **130–200 元** |
| 困難 | 220, 230, 240, 250, 260, 280, 280, 300 | **220–300 元** |
...
| 難度 | 各關預算 | 範圍 |
|------|---------|------|
| 簡單 | 70, 80, 80, 80, 100, 100, 100, 120 | **70–120 元** |
| 普通 | 140, 150, 160, 170, 180, 190, 200, 200 | **140–200 元** |
| 困難 | 220, 230, 240, 250, 260, 280, 280, 300 | **220–300 元** |
...
| 市場 | 簡單範圍 | 普通範圍 | 困難範圍 | 魔法商品金額上限* |
|------|---------|---------|---------|----------------|
| 傳統市場 🏪 | 60–100 元 | 110–180 元 | 180–260 元 | **110 元** |
| 超市購物 🛒 | 80–100 元 | 130–200 元 | 220–300 元 | **130 元** |
| 夜市美食 🏮 | 70–120 元 | 140–200 元 | 220–300 元 | **140 元** |

> \* 魔法商品金額上限 = 普通＋困難任務中最低的預算值（`_b6MagicMinBudget`）。超過此值的商品在最低預算關卡中永遠無法被選購，設定頁會阻擋輸入。
...
## 九、魔法商品功能（2026-04-29 全面重構，取代舊 customItems 系統）

- 設定頁「難度」下方有「✨ 魔法商品」切換列（普通/困難可見，`#b6-custom-items-toggle-row`）
- 啟用後**在設定頁**顯示魔法商品面板（`_renderMagicItemsPanel`），含攤位選擇 / 圖片上傳 / 名稱 / 金額（數字鍵盤）/ 單位
- 隨機市場（random）不支援魔法商品；市場類型切換時自動清空已設定的魔法商品
- 魔法商品儲存至 `state.settings.magicItems[]`（跨關保留；上限 5 個）
- **金額驗證**：設定時若超過該市場最低任務預算（`_b6MagicMinBudget`）則拒絕並提示
- `renderRound()` 對 `_currentStalls` 進行 deep copy 後注入魔法商品（原始資料不污染）
- `_calcMissionTotal()` 已移除 `customTotal`；魔法商品以 `selectedItems` 計入（與一般商品相同）
- 搜尋 `customItemsEnabled`、`magicItems`、`_renderMagicItemsPanel`、`_bindMagicItemsPanel`、`_b6MagicMinBudget`
...
| milk_tea_b | 奶茶 | icon-b6-milk-tea.png（共用飲料攤奶茶圖）| ✅ |
| custard | 優酪乳 | icon-b6-custard-yogurt.png | ✅ |

### 超市 — 冷凍區 frozen（12 項，11 項 ✅）
...
| mango_ice | 芒果冰 | icon-b6-mango-shaved-ice.png | ✅ |  改成 芒果冰沙
| plum_juice | 梅子汁 | icon-b6-plum-juice.png | ✅ |

### 夜市 — 紀念品攤 souvenir（14 項，7 項 ✅）
...
### 傳統市場（全部缺少）
**蔬菜攤（12）**：icon-b6-cabbage、icon-b6-tomato、icon-b6-scallion、icon-b6-sweet-potato、icon-b6-spinach、icon-b6-carrot、icon-b6-corn、icon-b6-cucumber、icon-b6-broccoli、icon-b6-pumpkin、icon-b6-daikon、icon-b6-pakchoi

**水果攤（12）**：icon-b6-apple、icon-b6-banana、icon-b6-grape、icon-b6-orange、icon-b6-melon、icon-b6-mango、icon-b6-watermelon、icon-b6-pineapple、icon-b6-strawberry、icon-b6-peach、icon-b6-papaya、icon-b6-guava

**雜貨攤（8）**：icon-b6-egg、icon-b6-soy-sauce、icon-b6-salt、icon-b6-sugar、icon-b6-cooking-oil、icon-b6-canned-food、icon-b6-dish-soap、icon-b6-tissue

### 超市（缺少 4）
icon-b6-muffin、icon-b6-cookie、icon-b6-butter、icon-b6-sm-egg（雞蛋，乳品區獨立圖）、icon-b6-ice-cream

### 夜市（缺少 8）
icon-b6-beef-noodle、icon-b6-taro-milk、icon-b6-sticker-set、icon-b6-charm、icon-b6-plush-toy、icon-b6-badge-pin、icon-b6-folding-fan、icon-b6-lanyard、icon-b6-tote-bag
