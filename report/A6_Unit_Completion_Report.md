...
> **更新日期**：2026-04-07（困難模式付款語音旗標控管 + step-hint 樣式更新）、2026-03-16（錢包組成多樣化 + 找零驗證正確/錯誤反饋一致性）
...
## 困難模式付款語音旗標控管 + step-hint 樣式更新（2026-04-07）

### 困難模式付款語音（`_showPaidAmount` 旗標）

**需求**：困難模式第五步（付款），拖曳放置金錢時不播放「你付了X元」語音，讓學生自行判斷；按下提示鈕後 `_showPaidAmount = true`，之後才播放。

**修改**（`js/a6_train_ticket.js`）：

- 桌面 drop 事件與觸控 drop 事件的普通/困難分支，在播放語音前加入：
  ```javascript
  ...
  ```
- `showPaymentHint()` 設定 `this._showPaidAmount = true`

搜尋：`_showPaidAmount`、`paymentSpeech`

### step-hint 提示動畫樣式更新

**需求**：普通模式步驟1確認起迄站頁面，錯誤3次後出現「點這裡」提示；emoji 改為👇，樣式改為仿 A3 橙色漸層膠囊，位置移至按鈕上方（不遮站名）。

**修改**（`css/a6_train_ticket.css`）：

- `.step-hint::after`：`content: '👇 點這裡'`；`top: -54px; bottom: auto`（上方）；橙色漸層 `linear-gradient(135deg, #F58025 0%, #FFa040 100%)`；`animation: bounceHint 1s ease-in-out infinite`
- 新增 `@keyframes bounceHint`（0%→50%：`translateY(-5px)`）
- `.train-type-card.step-hint::after`、`.count-display.step-hint::after` 同步更新 `top: -54px; bottom: auto`

搜尋：`bounceHint`、`step-hint::after`
