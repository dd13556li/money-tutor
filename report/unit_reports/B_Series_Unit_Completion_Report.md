...
> **更新日期**：2026-03-29（三十輪以後豐富化：B1 場景類別篩選、B1/B4 三商店/單位比價、B1~B6 輔助點擊模式、B2 日記主題、B3 目標類別、B5 派對主題、B6 市場類型 — 詳見附錄三十輪豐富化總覽）
> **更新日期**：2026-03-30（系列概覽表更新：JS/CSS 行數（Rounds 26-39 後最新值）；各單元獨立報告同步更新至 Rounds 29-39 豐富化）
...
|------|------|---------|------|---------|---------|---------|--------|------|
| B1 | 今天帶多少錢 | 拖曳放置錢幣（HTML5 DnD + 觸控） + AssistClick | easy/normal/hard | **1,516** | v3.8 | **1,029** | 166 | ✅ 完成 |
| B2 | 零用錢日記 | 選擇題 / 數字鍵盤 + AssistClick | easy/normal/hard | **1,669** | v3.8 | **737** | 203 | ✅ 完成 |
| B3 | 存錢計畫 | 月曆拖曳 + 選擇題 / 數字鍵盤 + 撲滿動畫 + AssistClick | easy/normal/hard | **2,863** | v4.5 | **1,729** | 93 | ✅ 完成 |
| B4 | 特賣比一比 | 選擇題 + 差額輸入 + 三商店排序 + 單位比價 + AssistClick | easy/normal/hard | **2,078** | v3.7 | **826** | 110 | ✅ 完成 |
| B5 | 生日派對預算 | 關卡制商品選購 + AssistClick | easy/normal/hard | **1,403** | v3.7 | **687** | 128 | ✅ 完成 |
| B6 | 菜市場買菜 | 關卡制採購 + 付款找零 + 三市場類型 + AssistClick | easy/normal/hard | **1,997** | v3.8 | **967** | 131 | ✅ 完成 |

> 資料更新日期：2026-03-30（Rounds 26-39 豐富化後）。JS 版本為 HTML 中 `?v=X.X` cache-busting 版號。
...
## 六十六、B3 月曆模式測驗語音完整設計（2026-03-25 第十四輪）

### 66.1 語音觸發完整清單

| # | 觸發時機 | 語音文字範例 | 設計目的 |
|---|---------|------------|---------|
| 1 | 彈窗顯示（存錢目標）| `存錢目標，玩具機器人，需要 300 元` | 建立學習目標、啟動動機 |
| 2 | 點擊「開始存錢！🐷」| `開始存錢` | 確認行動意圖 |
| 3 | 點擊日曆格（存錢圖示）| `4月2日，第3天` | 強化日期概念、計數意識 |
| 4 | 啟動拖曳後 | `今天可以存 20 元` | 確認當日金額、準備拖曳 |
| 5 | 拖放中途（非最後一枚）| `存入 10 元` | 即時面額回饋 |
| 6 | 拖放最後一枚（播完再下一步）| `存入 20 元` → callback → `_completeDragSession` | 確保聽完再進行結算 |
| 7 | 每次存完・達標 | `太棒了！存到 300 元了，可以買玩具機器人了！` | 成就感強化 |
| 8 | 完成畫面 | `太棒了！你只用了 15 天就存到了 300 元，買到了玩具機器人！` | 總結激勵 |
| 9 | 兌換按鈕 | `5個10元換成1個50元` | 面額兌換概念 |
| 10 | 語音重播鈕 | 重播 `c.lastSpeech` | 聽覺輔助 |

> **已移除**：`共存了 {累計金額}，還需要 {剩餘金額}`（視覺 UI 已涵蓋，避免冗餘語音）

### 66.2 關鍵語音設計原則

**① callback 驅動最後步驟**
```
最後一枚放置 → speak("存入X元", callback) → 語音 onend → _completeDragSession()
```
非最後枚：`speak("存入X元")` 無 callback，不阻塞後續拖放。

**② 日期語音（月日 + 第N天雙資訊）**
- 月份：`c.startDate.getMonth() + 1`（目前月曆顯示月份）
- 日期：`day`（點擊的日格數字）
- 第N天：`c.clickedDays + 1`（點擊時尚未遞增）

**③ 彈窗語音與按鈕語音分離**
- 彈窗出現 → 立即播放存錢目標語音
- 按鈕點擊 → 播放「開始存錢」後關閉彈窗
- **不用 callback 阻塞**：按鈕點擊後立即關閉彈窗，語音平行播放

### 66.3 對照 A/C/F 系列語音設計

| 特性 | B3 月曆 | A 系列 | C 系列 | F 系列 |
|------|--------|--------|--------|--------|
| 彈窗語音 | 出現時自動播放 | A1 任務彈窗自動播放 | C4 instruction modal 自動播放 | F1 任務彈窗自動播放 |
| 步驟確認語音 | 日期+第幾天 | A5 插卡/輸入步驟語音 | C6 進題語音 | F4 放置數字語音 |
| 拖放即時回饋 | 每枚「存入X元」| A3 每項餐點語音 | C2 每枚計數語音 | F3 每個放置語音 |
| 完成語音串接 | callback 驅動 | A 系列 speech callback chain | C3 completeExchange callback | F4 handleInstantFeedback callback |

---

## 六十七、B 系列測驗互動深化實作對照表（2026-03-25 最終版）

### 67.1 已實作的跨系列 Pattern 移植清單（完整版）

| # | 實作功能 | 目標單元 | 靈感來源 | 核心函數 | 實作日期 |
|---|---------|---------|---------|---------|---------|
| 1 | 開題任務說明彈窗 | B1 | C4 `_showInstructionModal` | `_showTaskModal(curr, diff)` | 2026-03-25 |
| 2 | 困難模式隱藏個別費用（??? 元）| B1 | C1 困難模式僅聽聲音 | `_renderScheduleCard` isHard 分支 | 2026-03-25 |
| 3 | 放幣語音反饋（coin.mp3 + 80ms 後語音面額）| B1 | F4 放置數字語音 + C1 硬幣識別 | `addCoin(denom)` TimerManager | 2026-03-25 |
| 4 | Easy 模式逐項動畫高亮 | B2 | C2 逐一點擊計數 | `_animateEasyEntries(question)` | 2026-03-25 |
| 5 | Easy 模式逐項小計顯示卡 | B2 | F5 量視覺化橫條 | `#b2-running-total`, `b2RtPop` | 2026-03-25 |
| 6 | 計算過程逐步算式提示 | B2 | C4/C6 分步驟提示 | `_showCalcBreakdown(question)` | 2026-03-25 |
| 7 | 月曆進度里程碑徽章（25/50/75%）| B3 | F2 里程碑音效 + A3 任務彈窗 | `_showMilestoneBadge(pct)` | 2026-03-25 |
| 8 | 除法公式提示（答錯顯示 ÷ 算式）| B3 | B2 計算過程 + B4 差額算式 | `_showDivisionHint(question)` | 2026-03-25 |
| 9 | 月曆彈窗語音（存錢目標 + 開始存錢）| B3 | A1 任務彈窗語音 + C4 instruction 語音 | `_showCalendarTaskPopup` Speech.speak | 2026-03-25 |
| 10 | 日期語音（×月×日，第×天）| B3 | A5 步驟語音 + C6 進題語音 | `_handleDayClick` Speech.speak | 2026-03-25 |
| 11 | 最後拖放 callback 驅動下一步 | B3 | A 系列 speech callback chain | `_handleCoinDrop` isLast 分支 | 2026-03-25 |
| 12 | 視覺價差比例橫條 | B4 | F5 量比較視覺橫條 | `_renderPriceBars(curr)` | 2026-03-25 |
| 13 | 差額算式提示（答錯顯示 − 算式）| B4 | A4 步驟 3 付款公式提示 | `_showDiffFormulaHint()` | 2026-03-25 |
| 14 | 完成畫面累計節省統計 | B4 | A4 交易摘要概念 | `quiz.totalSaved`, `.b4-savings-banner` | 2026-03-25 |
| 15 | 付款最佳面額提示 | B6 | A6 付款面額提示 + A4 hint | `_showPaymentHint(total)` | 2026-03-25 |
| 16 | 困難模式找零三選一 | B6 | C6 找零三選一 | `_showChangeQuiz(paid, total, change)` | 2026-03-25 |
| 17 | 正確商品彈出價格動畫 | B6 | A4 交易摘要商品動畫 + A3 點餐確認 | `_showPricePopup(anchor, price)` | 2026-03-25 |
| 18 | 攤位需求件數徽章（紅/綠）| B6 | C5 夠/不夠指示燈 | `.b6-stall-badge`, `.b6-stall-badge-done` | 2026-03-25 |
| 19 | 預算提示鈕（高亮可負擔商品）| B5 | B1 `_showCoinHint` 脈動高亮 | `_showBudgetHint()` | 2026-03-25 |
| 20 | 超支智慧移除建議（貪婪算法）| B5 | A1/A4 付款錯誤回饋引導 | `_handleConfirm` 超支分支 | 2026-03-25 |
| 21 | 關卡轉場卡（第 N 關動畫）| B5 | C6 `transitionText` 過渡 | `_showRoundTransition(roundNum, cb)` | 2026-03-25 |

### 67.2 各單元已實作 Pattern 數統計

| 單元 | Pattern 數 | 主要來源系列 |
|------|-----------|------------|
| B1 | 3 | C1, F4 |
| B2 | 3 | C2, C4/C6, F5 |
| B3 | 5 | A1, A3, A5, C4, C6, F2 |
| B4 | 3 | A4, F5 |
| B5 | 3 | A1, A4, B1, C6 |
| B6 | 4 | A3, A4, A6, C5, C6 |
| **合計** | **21** | A/C/F 全系列 |
...
## 六十八、B 系列 vs A/C/F 測驗特色完整對比（2026-03-25 更新版）

### 68.1 四大系列測驗核心特色對比

| 面向 | A 系列（生活應用）| C 系列（貨幣認知）| F 系列（數學基礎）| **B 系列（預算規劃）** |
|------|-----------------|-----------------|-----------------|---------------------|
| **核心主題** | 情境模擬（ATM/超市/販賣機）| 錢幣操作（辨識/數數/換錢）| 數學概念（對應/排序/組成）| 預算規劃（帶錢/記帳/存錢/比價）|
| **答題方式** | 拖曳 + 點選 + 鍵盤 | 拖曳 + 點選 + 聽覺 | 拖曳 + 點選 + 鍵盤 | 拖曳 + 點選 + 鍵盤 + 關卡選購 |
| **難度分級** | 3 級（引導/半引導/自主）| 3 級（視覺/文字/聽覺）| 3 級（輔助/標準/挑戰）| 3 級（引導/計算/精算）|
| **題數設計** | 固定（5/10/15/20）| 可選（1/3/5/10/自訂）| 可選（1/3/5/10/自訂）| 混合（B1-B4 可選；B5/B6 關卡制）|
| **完成畫面** | 紫色漸層 + 煙火 + 統計 | 紫色漸層 + 煙火 + 統計 | 紫色漸層 + 煙火 + 統計 | 紫色漸層 + 煙火 + 統計（統一）|
| **輔助點擊** | A1~A6 全支援（9~15 phases）| C4/C5 部分支援 | F1/F3 部分支援 | **尚未實作**（B6 設計規格已存在）|
| **自訂機制** | 飲料/餐點/商品/車站 | 無 | 主題圖片上傳 | B3 自訂物品圖片上傳 |
| **特殊互動** | 步驟化流程（A5 ATM）| 多輪兌換（C3）| 時間限制（F4/F5）| 月曆模式（B3）、關卡制（B5/B6）|

### 68.2 語音設計豐富度對比

| 語音設計層面 | A 系列 | C 系列 | F 系列 | B 系列 |
|------------|--------|--------|--------|--------|
| 進題語音 | ✅ 各難度模板 | ✅ 各難度模板 | ✅ 各難度模板 | ✅ 各難度模板 |
| 每步互動語音（拖放即時）| ✅ A3/A6 | ✅ C2/C3 | ✅ F3/F4 | ✅ B3/B1 |
| 日期/計數語音 | ❌ | ❌ | ✅ F2 逐數計數 | ✅ B3 日期語音 |
| 算式提示語音 | ❌ | ❌ | ❌ | ✅ B2/B3/B4 |
| 完成語音 | ✅ | ✅ | ✅ | ✅ |
| 里程碑語音 | ❌ | ❌ | ✅ F2 | ✅ B3 進度徽章 |
| callback 串接 | ✅ 全系列 | ✅ 全系列 | ✅ 全系列 | ✅ 全系列 |

### 68.3 B 系列相對其他系列的獨特貢獻

| # | B 系列獨有特色 | 教學意義 |
|---|-------------|---------|
| 1 | **月曆拖曳模式**（B3）| 全 Money Tutor 唯一跨日累積模擬，建立存錢習慣 |
| 2 | **手動面額兌換**（B3 EXCHANGE_RULES）| 在存錢情境自然學習面額換算（融合 C3 換錢概念）|
| 3 | **必買約束系統**（B5 鎖定必需品）| 「必要支出」概念，預算規劃基礎認知 |
| 4 | **超支智慧建議**（B5 貪婪算法）| 具體可行的超支回饋，非只說「不夠」|
| 5 | **關卡制選購**（B5/B6）| 連續 8~12 關，累積情境深度（對比 A/C/F 每題獨立）|
| 6 | **三攤位採購**（B6）| 生活化場景，跨類別預算管控 |
| 7 | **即時預算條**（B5 `b5-total-bar`）| 視覺化預算使用狀態（green/yellow/red 三態）|
| 8 | **累計節省統計**（B4 `totalSaved`）| 比價行為量化，強化省錢動機 |
| 9 | **兩段式比較流程**（B4 Select → Diff）| 先判斷貴/便宜，再計算差額，層次漸進 |
| 10 | **防慣性隨機交換**（B4 `swapped`）| 50% 機率交換左右，防止學生只按習慣選 |

### 68.4 B 系列下一步建議（對照 A/C/F 仍缺少的特色）

| 優先度 | 缺少的特色 | 對標來源 | 說明 |
|--------|---------|---------|------|
| 高 | 輔助點擊模式（B6）| A 系列全套 | B6 三段流程最適合輔助點擊；設計規格已在 §五十八 |
| 高 | B1 提示鈕（困難模式 3 次後）| A 系列普通模式 | 目前困難模式無提示機制 |
| 中 | B5/B6 圖片系統（真實商品圖）| B3 圖片系統 | 目前只有 emoji；B3 已有圖片架構可複製 |
| 中 | 計時挑戰選項 | F4/F5 計時模式 | B4/B6 最適合加計時 |
...
**主題**：B3 存錢統計摘要、B4 省錢排行榜、B6 攤位消費分析

### B3 — 存錢統計摘要（F5 統計卡片 pattern）

完成畫面第二頁「存錢目標清單」下方新增「🐷 存錢統計摘要」三格橫排：目標數量／合計目標金額／平均需要週數，讓學生直觀看到整局的積累成果。

| 位置 | 修改內容 |
|------|---------|
| **無新增 state** | 直接從既有 `q.achievedGoals` 計算三項摘要 |
| `showResults()` 第二畫面 | 在 `.b3-res-goals` 下方追加 `.b3-goal-summary` 三格橫排 |
| `css/b3_savings_plan.css` | `.b3-goal-summary`、`.b3-gs-item`、`.b3-gs-label`、`.b3-gs-val` |

視覺：淡紫底框，三欄：目標數（個）/ 合計金額（元）/ 平均週數（週）。

**搜尋**：`b3-goal-summary`、`b3-gs-item`

---

### B4 — 省錢排行榜（C1 統計 + A4 交易摘要 pattern）

完成畫面在「比價歷程」表格前新增「🏅 省錢排行榜」，排列本局省最多錢的前 3 名商品，附上獎牌與便宜商店名稱。

| 位置 | 修改內容 |
|------|---------|
| **無新增 state** | 從既有 `q.comparisonHistory` 排序取 top 3 |
| `showResults()` | 計算 `savingsRankHTML`，渲染 `.b4-res-ranking` 排行列表 |
| `css/b4_sale_comparison.css` | `.b4-res-ranking`、`.b4-rank-list`、`.b4-rank-row`、`.b4-rank-medal/item/store/saved` |

視覺：橘色底框，🥇🥈🥉 三行：獎牌+商品+便宜店+省X元。

**搜尋**：`savingsRankHTML`、`b4-res-ranking`、`b4-rank-medal`

---

### B6 — 攤位消費分析（B5 budget bars pattern）

完成畫面在「採購收據」後新增「🏪 攤位消費分析」橫條圖，顯示本局在蔬菜攤／水果攤／雜貨攤各花了多少錢及佔比。

| 位置 | 修改內容 |
|------|---------|
| `state.game` | 新增 `stallStats: {}` |
| `resetGameState()` | `g.stallStats = {}` |
| `_showChangeResult()` | items 構建迴圈中同步累計 `g.stallStats[stall] += item.price` |
| `showResults()` | 計算 `stallBreakdownHTML`，渲染 `.b6-res-stall-stats` 橫條圖 |
| `css/b6_market_shopping.css` | `.b6-res-stall-stats`、`.b6-stall-bars`、`.b6-stall-bar-row`、`.b6-stall-fill` |

視覺：黃框，每攤一行：攤位 emoji + 名稱 + 進度條（按佔比）+ 金額。

**搜尋**：`stallStats`、`b6-res-stall-stats`、`b6-stall-fill`

---

### 第二十二輪設計共通原則

- B3/B4：**無新 state**，從既有 `achievedGoals`/`comparisonHistory` 派生（最精簡模式）
- B6：輕量 state（一個 `{}` 物件）在已有的 `items` 建構迴圈中附帶累計
- 橫條圖比例計算：各攤位/各關佔 grand total 百分比，`Math.min(pct,100)%` 防溢出

---

## 第二十三輪豐富化（2026-03-27）

**主題**：B4 省錢 toast（遊戲中）、B6 關卡完成轉場卡（遊戲中）、B2 最大收支記錄（完成畫面）

### B4 — 省錢 toast 浮動提示（A4 `_showPricePopup` pattern）

每次 diff 答對後，立即彈出一個綠色浮動 toast「💰 省了 X 元！」從底部升起，1.1s 後自動淡出消失，視覺強化每題省錢的即時回饋。

| 位置 | 修改內容 |
|------|---------|
| `_showSavingsToast(amount)` | 新增函式：`position:fixed`，`bottom:110px`，`z-index:9900`，`b4ToastUp` 動畫 1.5s |
| `handleDiffAnswer()` 正確分支 | comparisonHistory push 後呼叫 `this._showSavingsToast(correctDiff)` |
| `css/b4_sale_comparison.css` | `@keyframes b4ToastUp`、`.b4-savings-toast`、`.b4-toast-fade` |

視覺：線性漸層綠底白字膠囊，底部固定位置，升起+淡出動畫，`pointer-events:none` 不干擾遊戲操作。

**搜尋**：`_showSavingsToast`、`b4-savings-toast`、`b4ToastUp`

---

### B6 — 關卡完成轉場卡（B5 `_showRoundTransition` pattern）

每關付款/找零完成後，點擊「下一關」時（非最後關），顯示深綠色全屏轉場卡「第N關完成！」列出採購商品、金額摘要，1.5s 後自動前進或點任意處跳過。最後關直接進完成畫面。

| 位置 | 修改內容 |
|------|---------|
| `_showRoundCompleteCard(roundNum, items, total, paid, change, callback)` | 新增函式：全屏覆蓋卡，`b6RcIn` 彈出動畫，click 或 1.5s timer 觸發 callback |
| `_showChangeResult()` 下一關 click handler | 改為：非最後關呼叫 `_showRoundCompleteCard(…)`，最後關呼叫 `this.nextRound()` |
| `css/b6_market_shopping.css` | `@keyframes b6RcIn`、`.b6-round-complete`、`.b6-rc-inner/.b6-rc-badge/emoji/items/amounts/hint` |

視覺：深綠半透明全屏，彈出卡含關卡號、🎉、商品列表、付款摘要、「點任意處繼續」提示。

**搜尋**：`_showRoundCompleteCard`、`b6-round-complete`、`b6RcIn`

---

### B2 — 最大收支記錄（C1 統計 + A4 highlight pattern）

完成畫面「本期收支總計」後新增「📌 本期最大記錄」，顯示本局最大單次收入（事件名+金額）與最大單次支出（事件名+金額），讓學生回顧金額差異最大的理財決策。

| 位置 | 修改內容 |
|------|---------|
| **無新 state** | 從既有 `q.answeredHistory[].events[]` 遍歷求最大值 |
| `showResults()` | 計算 `maxRecordsHTML`，插入 `incomeTotalHTML` 後 |
| `css/b2_allowance_diary.css` | `.b2-res-max-records`、`.b2-max-row`、`.b2-max-item.income/expense`、`.b2-max-icon/label/name/val` |

視覺：灰框，左綠（最大收入）右紅（最大支出），含事件 icon + 事件名稱 + 金額。

**搜尋**：`maxRecordsHTML`、`b2-res-max-records`、`b2-max-item`

---

### 第二十三輪設計共通原則

- **B4/B6**：首次加入**遊戲中視覺反饋**（非完成畫面），對齊 A4 `_showPricePopup` 和 B5 轉場卡模式
- **B2**：延續第 21 輪「從既有 state 派生」最精簡模式，**無新欄位**
- 遊戲中 toast/轉場卡均使用 `z-index: 9800/9900`，不干擾標題列（z:100）或彈窗（z:10100）
...
**本輪主題**：開場引導卡、連勝里程碑、剛好付款提示

---

### B5 — 每關開場預算介紹卡（B1 `_showTaskModal` + B5 轉場卡 pattern）

每關遊戲畫面渲染完成後，立即在畫面上方疊加一張全屏半透明卡片，顯示「第 N 關」和本關預算金額，並朗讀語音。學生點任意處或等 1.8 秒後自動關閉，進入選購。

| 位置 | 修改內容 |
|------|---------|
| `renderRound()` | 移除舊語音 setTimeout；末尾呼叫 `this._showRoundIntroCard(g.currentRound + 1, scenario.budget)` |
| `_showRoundIntroCard(roundNum, budget)` | 新增：紫色半透明全屏蓋板；白色圓角卡顯示關卡 + 🎂 + 預算大字；語音整合；1800ms 後或點擊後 fade 消失 |
| `css/b5_party_budget.css` | `.b5-round-intro`（`b5RiIn` 彈出動畫）、`.b5-ri-inner/round/icon/label/budget/hint` |

視覺：紫色毛玻璃背景，白卡，圓形 🎂，橘色 44px 預算數字，底部淺灰提示文字。

**搜尋**：`_showRoundIntroCard`、`b5-round-intro`、`b5RiIn`

---

### B3 — quiz 模式答對連勝徽章（B3 `_showMilestoneBadge` + F2 里程碑 pattern）

quiz 模式（困難）答對連續 3 題顯示「🔥 3連勝！」徽章，連續 5 題顯示「⚡ 5連勝！」徽章。答錯立即歸零。

| 位置 | 修改內容 |
|------|---------|
| `state.quiz` | 新增 `streak: 0` 欄位 |
| `resetGameState()` | `q.streak = 0` |
| 困難模式 quiz 啟動 | `q.streak = 0` |
| `_handleChoiceAnswer()` | 正確：`streak++`；達 3/5 觸發 `_showStreakBadge`；錯誤：`streak = 0` |
| `_handleNumpadAnswer()` | 同上 |
| `_showStreakBadge(streak)` | 新增：橘色漸層 badge 置中彈出；語音「三連勝，繼續加油！」/「五連勝，太厲害了！」；1.6s 後 fade |
| `css/b3_savings_plan.css` | `.b3-streak-badge`（`b3SbPop` 彈出）、`.b3-sb-inner/label/msg` |

視覺：`linear-gradient(135deg, #f59e0b, #d97706)` 橘金色，32px 粗體 emoji 標籤，白色副標。

**搜尋**：`_showStreakBadge`、`b3-streak-badge`、`b3SbPop`、`streak`

---

### B1 — 投幣剛好浮動提示（B4 toast + B6 `wasSufficient` pattern）

當投入硬幣總額**恰好等於**行程費用時（不多不少），在畫面底部顯示「💯 剛好！不需要找零」浮動 toast，並播放對應語音。普通的「金額足夠」只說「可以出發」，剛好時才觸發此 toast。

| 位置 | 修改內容 |
|------|---------|
| `_updateWalletDisplay()` | `enough && !wasSufficient && total > 0` 分支中加 `total === required` 判斷；剛好→`_showExactMatchToast()` + 語音「剛好！不需要找零，可以出發了！」；不剛好→原語音 |
| `_showExactMatchToast()` | 新增：底部居中綠色 toast；1.4s 後 fade 消失；防重複 `existing.remove()` |
| `css/b1_daily_budget.css` | `.b1-exact-toast`（`b1ToastUp` 上飄彈出）、`.b1-toast-fade` |

視覺：深綠漸層，40px 圓角，18px 粗體，底部 80px 偏移（不遮確認按鈕）。

**搜尋**：`_showExactMatchToast`、`b1-exact-toast`、`b1ToastUp`

---

### 第二十四輪設計共通原則

- **B5**：開場引導卡讓預算資訊更醒目，語音整合移至卡片，消除原先 400ms 延遲計時器
- **B3**：連勝徽章採「不記新結果 state，只記 streak 計數」最精簡模式；觸發點（3/5）對應 A4 商店等級概念
- **B1**：toast 為 `pointer-events:none`，不阻擋學生繼續操作；剛好付款是數學精算能力的正向強化
...
# 附錄：B 系列測驗內容特色與單元規格（2026-03-28）

> 本附錄參照 F/C/A 系列 report 格式，統整 B 系列六個單元的教學目標、測驗內容、互動方式與技術架構，作為未來開發與維護的參考基準。

---

## 壹、系列定位與學習路徑

### B 系列在 Money Tutor 課程中的位置

```
F 系列（數學基礎：唱數、辨數、排序、量比較、數的組成）
    ↓
C 系列（貨幣認知：認識錢幣→數錢→換錢→付款→判斷→找零）
    ↓
【B 系列：預算規劃】← 本系列
  B1 今天帶多少錢  B2 零用錢日記  B3 存錢計畫
  B4 特賣比一比   B5 生日派對預算 B6 菜市場買菜
    ↓
A 系列（生活應用：販賣機→理髮廳→麥當勞→超市→ATM→火車票）
```

**B 系列核心定位**：將 C 系列學到的「認識面額、數錢、找零」能力，應用於「規劃預算、記帳、儲蓄、比價、採購」等真實生活決策情境。

### 各單元核心數學概念對應

| 單元 | 情境 | 核心能力 | 數學概念 |
|------|------|---------|---------|
| B1 今天帶多少錢 | 出遊行程規劃 | 加總費用、選對面額 | 加法、最佳面額組合 |
| B2 零用錢日記 | 記錄每週收支 | 追蹤餘額變化 | 加減法、正負數 |
| B3 存錢計畫 | 目標儲蓄規劃 | 計算達標週數 | 除法（無條件進位） |
| B4 特賣比一比 | 跨店比價 | 判斷便宜、算差額 | 減法、比大小 |
| B5 生日派對預算 | 固定預算選購 | 不超支地選商品 | 加法累計、預算管理 |
| B6 菜市場買菜 | 多攤位採購找零 | 採購+付款+找零 | 多步驟加減運算 |
...
> 「今天要出去玩！看看行程表，計算帶多少錢，然後把正確的硬幣放進錢包。」

學生面對一張行程卡（含 3~5 個活動及費用），需要：
①加總費用 → ②選對面額 → ③拖曳硬幣放進錢包

#### 難度遞進設計

| 難度 | 可用面額 | 費用可見性 | 金額範圍 | 特殊挑戰 |
|------|---------|----------|---------|---------|
| 簡單 | 1/5/10/50 元 | 全部可見 | 20~120 元 | 小面額組合練習 |
| 普通 | +100/500 元 | 全部可見 | 50~500 元 | 跨面額混合練習 |
| 困難 | +1000 元 | **個別費用隱藏** (???元) | 100~2000 元 | 聽語音記憶加總 |

**困難模式設計精髓**：個別費用顯示為 `??? 元`（灰色斜體），學生必須依賴語音播報的金額自行記憶加總，訓練工作記憶與聽覺注意力。

#### 測驗互動方式

- **主互動**：拖曳硬幣（桌面 HTML5 DnD / 觸控 TouchDragUtility）
- **投幣回饋**：每枚硬幣落入錢包立即語音報額（「50元」「100元」）
- **足額提示**：到達目標金額語音說「金額足夠，可以出發了！」
- **剛好提示**：剛好等於目標時浮動 toast「💯 剛好！不需要找零！」
- **提示鈕**：高亮建議面額的硬幣按鈕（脈動動畫）

#### 完成畫面資訊

- 🏆 表現評價（≥90%/≥70%/≥50%/<50%）
- 📊 面額使用統計：每種面額使用次數格子
- 📋 完成行程清單：emoji + 名稱 + 各項費用 + 合計
- 答對題數 / 正確率 / 時間
...
> 「這週的零用錢日記，記錄了幾件收支事件——從起始金額開始，最後還剩多少錢？」

學生面對日記事件卡（收入綠色/支出紅色），需要：
①識別收入/支出方向 → ②依序計算 → ③選出或輸入最終餘額

#### 難度遞進設計

| 難度 | 事件數 | 答題方式 | 特殊設計 |
|------|-------|---------|---------|
| 簡單 | 2 件 | 選擇題（retry） | 逐項動畫高亮 + 即時小計卡 |
| 普通 | 4 件 | 選擇題（retry） | 一次顯示，可重試 |
| 困難 | 6 件 | 鍵盤數字輸入 | 一次顯示，只能進行不重試 |

**簡單模式動畫流程**（`_animateEasyEntries`）：
1. 初始隱藏選項
2. 每 800ms 高亮一個日記事件（黃底縮放動畫）
3. 同步更新「目前餘額」數字卡（收入→綠色彈出 / 支出→紅色彈出）
4. 同步語音播放（「收入 X 元」/「花了 X 元」）
5. 所有事件顯示完畢後 500ms 淡入選項

#### 測驗互動方式

- **選擇題**：4 個選項（含干擾值），答錯可重試；≥3 次錯誤自動顯示逐步計算過程
- **鍵盤輸入**：純數字鍵盤，答錯顯示提示後繼續下一題
- **開題彈窗**：「本週零用錢，起始 X 元」（橘色大字，語音同步）
- **計算過程提示**：逐步算式（起始 ± 事件 ＝結果），綠=收入、紅=支出

#### 完成畫面資訊

- 📒 記帳日記回顧：綠框斑馬表，收入綠膠囊 / 支出紅膠囊
- 💰 本期收支總計：三欄（總收入綠 / 總支出紅 / 淨餘額藍或黃）
- 📌 本期最大記錄：最大單筆收入 / 最大單筆支出
...
> 「想買一個玩具，每週能存一定的錢——需要幾週才能買到？」

B3 是 B 系列中**功能最豐富的單元**，包含兩種完全不同的互動體驗：
- **月曆模式（簡單）**：拖曳金幣存入撲滿，感受儲蓄的時間積累過程
- **計算模式（普通/困難）**：用除法算出「需要幾週」

#### 難度遞進設計

| 難度 | 模式 | 物品金額 | 週存選項 | 答題方式 |
|------|------|---------|---------|---------|
| 簡單 | 月曆互動 | 依設定選擇 | — | 拖曳金幣放入投槽 |
| 普通 | 計算模式 | ≤800 元 | 50~200 元/週 | 4 選 1 |
| 困難 | 計算模式 | 全部範圍 | 50~500 元/週 | 鍵盤輸入 |

**月曆互動模式流程**：
點擊日曆格 → 顯示「今日可存金錢」 → 拖曳正確面額金幣 → 面額匹配✓→ 撲滿更新 → 進度條前進 → 達里程碑（25/50/75%）→ 徽章動畫 → 達 100% 完成

**撲滿兌換系統**（EXCHANGE_RULES，6條規則）：
- 10個1元 → 1個10元
- 5個10元 → 1個50元
- 2個50元 → 1個100元
- 5個100元 → 1個500元
- 2個500元 → 1個1000元
- 10個100元 → 1個1000元
- 達閾值時顯示綠色「🔄 X個換1個Y元」手動兌換按鈕

#### 計算模式核心公式

```
需要幾週 = ⌈目標金額 ÷ 每週存款⌉（無條件進位）
```

答錯時顯示除法提示：「X元 ÷ Y元/週 ≈ Z週（無條件進位）」

#### 完成畫面資訊

- 🐷 存錢目標清單：物品圖示 + 名稱 + 售價 + 每週存X元×Y週
- 統計摘要三格：目標數量 / 合計金額 / 平均週數
...
> 「同一件商品，A 店和 B 店賣不同的價格——哪家比較便宜？便宜多少元？」

B4 是 B 系列中唯一採用**兩段式答題流程**的單元：
1. **第一段 Select**：選擇哪家店較便宜（4 選 1）
2. **第二段 Diff**：計算差額是多少元（選擇題或鍵盤）

#### 難度遞進設計

| 難度 | Select 方式 | Diff 方式 | 輔助功能 |
|------|-----------|---------|---------|
| 簡單 | 選擇題 | 選擇題 | 視覺比例條、差額算式提示（答錯後） |
| 普通 | 選擇題 | 選擇題 | 同上 |
| 困難 | 選擇題 | 選擇題 | 困難模式提示鈕（`💡 提示`） |

**視覺比例條設計**（`_renderPriceBars`）：
- 雙欄橫條，依實際價格比例顯示相對寬度
- 貴的那欄：紅漸層 / 便宜的那欄：綠漸層
- 在 Diff 階段前幫助學生直觀感受「哪個貴多少」

#### 測驗互動方式

- **Select 答題**：4 個選項（A/B/C/D 店名按鈕）
- **Diff 答題**：4 個數字選項，或鍵盤輸入
- **差額算式提示**（答錯後）：顯示「XXX − YYY = ？ 元」
- **選錯提示**（≥3 次）：語音說出正確店名 + 脈動高亮
- **省錢 Toast**（答對後）：底部浮動「💰 省了X元！」

#### 完成畫面資訊

- 💰 節省橫幅：累計本次測驗節省的總金額
- 🏅 省錢排行榜：Top 3 medals（從 comparisonHistory 排序）
- 🛒 比價歷程表：4欄（商品 / 便宜店 / 較貴店 / 省下）
...
> 「要辦生日派對！有固定預算，必買物品已確定，再從可選清單中挑選——不能超支！」

B5 是 B 系列中唯一採用**關卡制（rounds）**的單元，每關情境獨立，考驗在固定預算內的判斷與決策能力。

#### 難度遞進設計

| 難度 | 關卡數 | 可選物品 | 預算緊張度 | 特殊設計 |
|------|-------|---------|----------|---------|
| 簡單 | 5~6 關 | 5~6 組 | 充裕（易達標）| 不顯示超支警告 |
| 普通 | 12 關 | 12 組 | 適中 | 標準流程 |
| 困難 | 8 關 | 8 組 | 緊張（必買已佔大部分）| 嚴格預算管理 |

**每關流程**：
```
開場介紹卡（關卡號 + 🎂 + 預算大字）
  → 必買物品區（鎖定，顯示√，不可取消）
  → 可選物品區（點擊 toggle 選/取消）
  → 即時更新「已使用金額」
  → 確認選購
    ├── 未超支 → 語音肯定 → 轉場卡 → 下一關
    └── 超支   → 紅色警告 + 智慧建議（高亮1個可移除項）
  → 全部完成 → showResults()
```

#### 測驗互動方式

- **點擊 toggle**：點擊可選物品卡片即選/取消（語音「{名稱}，{價格}元」/「取消{名稱}」）
- **預算提示鈕**：高亮可負擔未選商品 + 語音「還剩X元可加選Y」
- **超支智慧建議**：貪婪演算法找出單步最佳移除項，標記黃框 + 高亮閃爍
- **關卡轉場卡**：紫色全屏覆蓋「第X關」語音，1.4s 後進入下一關

#### 完成畫面資訊

- 🎉 本次派對採購物品：粉色標籤泡泡（收集所有答對關的商品）
- 📊 各關預算使用橫條圖：ok=綠 / near=橙（>80%）/ over=紅
...
> 「去菜市場買菜！跨多個攤位找齊任務清單上的食材，付款，計算找零。」

B6 是 B 系列中**流程最長、互動最複雜**的單元，整合了採購任務、攤位切換、付款計算、找零確認等多個環節。

#### 難度遞進設計

| 難度 | 攤位數 | 商品數 | 找零方式 | 提示可用性 |
|------|-------|-------|---------|---------|
| 簡單 | 2~3 | 3~5 件 | 直接顯示找零金額 | 付款最佳面額提示 ✓ |
| 普通 | 3~4 | 4~6 件 | 直接顯示找零金額 | 付款最佳面額提示 ✓ |
| 困難 | 4 | 5~7 件 | **三選一（偏移干擾項）** | 無提示鈕 |

**攤位分類系統**（B6_STALLS）：
- 🥬 蔬菜攤、🍎 水果攤、🥩 肉攤、🥚 乾貨攤
- 攤位分頁切換（stall tab），切換時語音播報攤位名稱
- 攤位需求徽章：待採購件數（紅色）→ 全收完（綠✓）
...
顯示採購任務清單（跨攤位）
  ...
  → 進入結帳付款
    ↳ 選面額付款（貪婪算法提示最少面額）
    ↳ 足額後語音「金額剛好！」/ 「超過，找零後可付款！」
  → 找零確認
    ├── 簡單/普通：顯示找零金額 + 語音
    └── 困難：三選一選項（含2個偏移干擾值）
  → 關卡完成轉場卡（非最後關）/ 直接完成（最後關）
```
...
- **採購**：點擊清單中的商品項目（找到即打√）
- **付款**：點擊面額圖片累積付款金額
- **找零（困難）**：三個選項（正確 / 正確+偏移1 / 正確+偏移2）
- **清除付款**：語音「清除，重新選擇」

#### 完成畫面資訊

- 🧾 採購收據表格：5欄（關卡 / 商品 / 小計 / 付款 / 找零）
- 🏪 攤位消費分析：各攤位佔比橫條圖（黃框）
...
所有 B 系列單元遵循相同架構（對齊 C/F 系列）：

```javascript
Game = {
    Debug: { FLAGS, log(), warn(), error() },         // 除錯旗標系統
    TimerManager: { setTimeout(), clearByCategory() }, // 計時器管理
    EventManager: { on(), removeByCategory() },        // 事件管理
    Speech: { speak(), speakCustom() },                // 語音系統
    Audio: { playCorrectSound(), playErrorSound() },   // 音效系統
    state: { settings, quiz, game },                   // 集中狀態管理
    init(), start(), showSettings(), generateQuestion(), // 核心邏輯
    nextQuestion(), showResults(), handleConfirm()      // 流程控制
}
```
...
| 互動方式 | 拖曳/點選 | 選擇題 | 鍵盤輸入 |
| 提示可用 | 最大輔助 | 部分輔助 | 最少輔助 |
| 答題機制 | retry（可重試）| retry/proceed | proceed（不重試）|

### 三、語音系統設計

所有 B 系列單元使用 `number-speech-utils.js` 的 `convertToTraditionalCurrency()` 做金額語音，包裝為：
```javascript
const toTWD = v => typeof convertToTraditionalCurrency === 'function'
    ? convertToTraditionalCurrency(v) : `${v}元`;
```

語音雅婷優先（`Microsoft Yating`），語速 `utterance.rate = 1.0`。
...
→ 表現評價徽章（≥90%🏆 / ≥70%👍 / ≥50%💪 / <50%📚）
→ 3 個統計卡片（答對題數 / 正確率 / 時間）
→ [各單元專屬資訊]（面額統計/日記回顧/存錢清單/排行榜/收據等）
→ 綠色「再玩一次」+ 紫色「返回設定」
→ 煙火動畫（canvas-confetti）
```

### 五、設定頁設計原則（2026-03-21 確立）

- 所有選項初始無預選（`null`）
- `_checkCanStart()` 要求全選才啟動
- 題數選項：5/10/15/20 題（B1~B4）；3/5/8 關（B5/B6）
- 答題方式：重試 / 繼續（B1~B4）；關卡制無此設定（B5/B6）

---

## 肆、B 系列 vs F/C/A 系列特色對比

| 比較維度 | F 系列 | C 系列 | A 系列 | B 系列 |
|---------|-------|-------|-------|-------|
| **核心學習** | 數學基礎概念 | 貨幣面額認知 | 生活場景操作 | 預算規劃決策 |
| **互動深度** | 拖曳配對 | 點擊選答 | 多步驟操作流程 | 多元互動混合 |
| **難度標誌** | 物品數量/類型 | 感知模態（視→聽）| 輔助程度遞減 | 資訊隱蔽度遞增 |
| **場景真實度** | 抽象（數量對應）| 半具象（面額辨識）| 具象（場所模擬）| 具象（生活決策）|
| **題庫結構** | 組態生成 | 固定題庫隨機抽 | 動態情境生成 | 固定情境+隨機化 |
| **完成畫面** | 3統計卡 | 3統計卡 | 3統計卡+學習成果 | 3統計卡+豐富回顧 |
| **語音深度** | 數量/順序 | 面額/數量 | 全流程指引 | 情境/反饋/強化 |

---

*本附錄由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-28*

---

## 伍、B 系列各單元測驗設計詳析（參照 F/C/A 格式）

> 本節依照 F/C/A 系列報告格式，逐單元詳述教學目標、測驗內容、測驗方式、互動特色與題庫設計。
> 更新日期：2026-03-29

---

### 5.1 B1 今天帶多少錢 — 情境式預算計算

#### 教學定位

**學習目標**：學生能根據當日行程費用需求，從錢包中選取正確面額與數量的錢幣，完成「出門帶夠錢」的情境模擬。

**前置知識**：認識各面額錢幣（C1~C3）→ 能數出指定金額（C2/C4）→ 了解生活費用概念

**核心能力訓練**：
1. 閱讀情境卡片，解讀多項費用條目
2. 計算各費用加總（多數字加法）
3. 從錢幣盤中選取對應面額湊出總額
4. 比較「帶的錢」與「需要的錢」是否相符

#### 核心測驗內容

**測驗主題**：8 大生活情境（上課日 / 公園 / 圖書館 / 診所 / 游泳 / 電影 / 生日派對 / 超市）

**費用項目設計**：
| 情境 | 項目數 | 費用範圍 | 情感連結 |
|------|-------|---------|---------|
| 公園一日遊 | 3~4 | 15~120 元 | 親子同樂 |
| 學校活動日 | 2~3 | 20~80 元 | 日常生活 |
| 醫院看病 | 2~3 | 50~150 元 | 日常生活 |
| 看電影 | 3~4 | 60~200 元 | 休閒娛樂 |
| 生日派對 | 3~5 | 80~300 元 | 特殊場合 |

**困難模式特例**：個別費用顯示「??? 元」，僅語音播報，學生需記憶計算。

#### 測驗方式與互動設計

| 步驟 | 學生動作 | 系統回饋 |
|------|---------|---------|
| ① 讀取情境 | 閱讀（簡單模式自動語音播報）| 行程費用條目展示 |
| ② 拖曳錢幣 | 從錢幣盤拖至錢包 | 即時金額更新 + 語音報數 |
| ③ 達到足額 | 系統自動判定 | 綠光脈動 + 「金額足夠！」語音 |
| ④ 確認付款 | 點擊確認按鈕 | 煙火 + 正確音效 + 完成 |

**特殊互動機制**：
- 拖曳操作：HTML5 Drag & Drop（桌面）+ Touch Events（行動裝置）
- 剛好付清：底部 toast「剛好！不需要找零」+ 行程卡綠光脈動（`b1ExactGlow`）
- 最少張數提示：答對後若用幣數超過最省方案，顯示橘色 toast「其實最少只需要 M 張」

#### 難度遞進設計

| 維度 | 簡單 | 普通 | 困難 |
|------|------|------|------|
| 費用可見性 | 全部顯示 | 全部顯示 | 個別費用顯示「??? 元」|
| 提示機制 | 任務彈窗自動 2 秒 | 任務彈窗（點提示鈕）| 任務彈窗（點提示鈕）|
| 面額範圍 | 1/5/10/50 元 | 1/5/10/50/100 元 | 全面額含紙鈔 |
| 費用項目數 | 2~3 項 | 3~4 項 | 4~5 項 |
| 答題方式 | 可重試 | 可重試 | 繼續（不可重試）|
| 費用明細提示 | 答錯 3 次後顯示 | 答錯 3 次後顯示 | 答錯後立即顯示 |
| 最少張數提示 | ✓（答對後）| ✓（答對後）| ✓（答對後）|
...
| 項目 | 規格 |
|------|------|
| 每難度題數 | 12 題（可選 5/10/15/20 題）|
| 金額範圍 | 簡單 50~200元 / 普通 80~350元 / 困難 150~600元 |
| 防重複機制 | 同輪不重複抽取同一情境 |
| 錢包組成 | 依總金額動態生成，含多面額隨機分配 |

#### 特色設計亮點

| 特色 | 說明 |
|------|------|
| 情境驅動 | 費用與真實生活場景結合，強化學習動機 |
| 貪婪最少張數算法 | `_calcOptimalCoins()` 提供「最省張數」教育性反饋 |
| 行程卡精準綠光 | 剛好付清時視覺強化「精確」概念 |
| 困難模式聽算 | 費用隱藏 → 學生依賴語音記憶計算，提升聽覺處理能力 |
| 面額使用統計 | 完成畫面顯示各面額使用次數，延伸 C1 面額認知 |
| 行程費用清單 | 完成畫面展示所有答對情境的費用明細 |

---

### 5.2 B2 零用錢日記 — 日記帳本加減運算

#### 教學定位

**學習目標**：學生能閱讀一週的收支日記，計算「本週剩餘金額」，訓練多步驟連續加減法能力。

**前置知識**：數字加減（F6）→ 認識收入與支出概念 → 能執行多步驟加減

**核心能力訓練**：
1. 閱讀記帳條目（日期、事件說明、金額）
2. 區分收入（+）與支出（−）
3. 從起始金額開始，逐步累加收入並扣除支出
4. 計算最終餘額
...
| 類型 | 顏色 | 圖示 | 舉例 | 金額範圍 |
|------|------|------|------|---------|
| 收入 | 綠色 | 💰📩🎁 | 零用錢 / 壓歲錢 / 家事獎勵 | +20~200 元 |
| 支出 | 紅色 | 🥤🍔📚 | 飲料 / 文具 / 零食 | −10~80 元 |

**三種難度題目設計**：
| 難度 | 事件數 | 起始金額 | 計算複雜度 |
|------|-------|---------|---------|
| 簡單 | 2 個 | 50~150 元 | 2 步加減 |
| 普通 | 4 個 | 80~200 元 | 4 步加減 |
| 困難 | 6 個 | 100~300 元 | 6 步加減（含可能負餘額）|

#### 測驗方式與互動設計

**簡單模式（選擇題）**：
1. 日記條目依序動畫出現（每 800ms 一條，`.b2-entry-active` 黃底縮放）
2. 每出現一條，即時小計卡同步更新（`.b2-running-total`，收入綠 / 支出紅）
3. 全部顯示後呈現 4 個選項（選項差距 ≥ 20 元，確保可分辨）
4. 答對：進行下一題；答錯：可重試

**普通 / 困難模式（鍵盤輸入）**：
1. 所有事件條目一次全部顯示
2. 學生使用數字鍵盤輸入答案 → 確認送出
3. 答錯後顯示逐步計算算式（`_showCalcBreakdown`）
4. 困難模式答錯不重試，直接進下一題
...
|------|------|------|------|
| 事件數 | 2 | 4 | 6 |
| 答題方式 | 四選一選擇題 | 數字鍵盤輸入 | 數字鍵盤輸入 |
| 動畫輔助 | 逐項動畫 + 即時小計 | 全部一次顯示 | 全部一次顯示 |
| 答錯處理 | 可重試 | 顯示算式 → 可重試 | 顯示算式 → 直接下一題 |
| 連續錯誤 | 錯 3 次 → 自動顯示算式 | 錯 1 次 → 顯示算式 | — |
| 開題彈窗 | 起始金額彈窗 2.2s | 同簡單模式 | 同簡單模式 |

#### 完成畫面特色資訊

| 區塊 | 內容 |
|------|------|
| 📒 記帳日記回顧 | 所有已答題目的完整條目展示（斑馬行）|
| 💰 本期收支總計 | 收入合計（綠）/ 支出合計（紅）/ 淨餘額（藍/黃）|
| 📌 本期最大記錄 | 最大單筆收入 + 最大單筆支出 |

---

### 5.3 B3 存錢計畫 — 除法規劃與月曆模擬

#### 教學定位

**學習目標（雙模式）**：
- **測驗模式（普通/困難）**：學生能計算「每週存 X 元，幾週後才能買到目標商品」，掌握除法與無條件進位
- **月曆模式（簡單）**：學生透過拖曳錢幣到月曆格，直觀體驗「每天積累一點，最終達成目標」的存錢過程

**前置知識**：加法 / 乘法（F6）→ 除法概念（F6）→ 認識紙鈔面額（C1）

**核心能力訓練**：
1. 讀取目標商品售價與每週存款金額
2. 執行除法運算（售價 ÷ 週存金額）
3. 理解無條件進位（存的週數必須「夠多」，不能剛好小數）
4. 月曆模式：辨識面額、拖曳操作、理解累積概念
...
| 價格區間 | 舉例 | 對應難度 |
|---------|------|---------|
| 200~500 元 | 玩具車 / 圖書 / 小型遊戲 | 簡單（3~4 週）|
| 500~1,500 元 | 電動玩具 / 球類用品 | 普通（5~10 週）|
| 1,500~3,500 元 | 電子設備 / 樂器 | 困難（10~20 週）|

**題目格式**：「一個 XXX 要 Y 元，每週存 Z 元，需要幾週才能買到？」
...
**互動流程**：
1. 顯示目標商品（圖示 + 售價）與每日可存金額
2. 點擊月曆中的「存錢格子」→ 啟動拖曳會話
3. 拖曳來源卡的面額金幣到月曆格
4. 正確面額：勾勾動畫 + 加入撲滿統計
5. 累積達到目標金額 → 完成動畫（煙火）
...
**撲滿兌換系統**（手動）：
- 記錄各面額數量（`denomPile`）
- 達到兌換閾值顯示「🔄 X 個換 1 個 Y 元」按鈕
- 6 條兌換規則（10×1→10、2×5→10、10×10→100 等）

#### 測驗方式比較

| 難度 | 模式 | 答題方式 | 提示機制 |
|------|------|---------|---------|
| 簡單 | 月曆模式 | 拖曳金幣到日曆格 | 進度條 + 里程碑徽章（25/50/75%）|
| 普通 | 測驗模式 | 四選一選擇題 | 除法提示（答錯後：Y÷Z≈週）|
| 困難 | 測驗模式 | 數字鍵盤輸入 | 除法提示（答錯後顯示）|

**週數預覽方塊**（答題時即時更新）：
- 選擇選項或輸入數字後，立即渲染對應週數的小方塊矩陣
- 最多顯示 16 格，超出顯示「+N」溢出標籤
- 答對時方塊轉綠（`b3BlockPop` 動畫），視覺化「數出幾週」

**開題目標彈窗**：每題顯示商品圖示 + 名稱 + 售價，語音「存錢目標：XXX，YYY 元」，2.5 秒自動關閉。

#### 特色設計亮點

| 特色 | 說明 |
|------|------|
| 雙模式並行 | 月曆體驗（感受）vs 計算題（理解），同一概念兩種呈現 |
| 除法無條件進位強調 | 週數必須「足夠多」，教導現實規劃的嚴謹性 |
| 月曆進度里程碑 | 25/50/75% 各觸發徽章，正增強學習動力 |
| 真實商品圖片 | 增強目標感，強化「存錢是為了買東西」的因果連結 |
| 手動兌換系統 | 模擬換鈔概念，延伸 C3 兌換知識至日常情境 |
| 自訂存錢物品 | 家長/老師可上傳客製化目標商品圖片 |

---

### 5.4 B4 特賣比一比 — 比較判斷與差額計算

#### 教學定位

**學習目標**：學生能比較同一商品在不同店家的售價，找出最便宜選項，並計算兩店之間的差額（節省多少錢）。

**前置知識**：數字大小比較（F4）→ 減法差額（F6）→ 認識商品價格

**核心能力訓練**：
1. 比較兩個（或三個）數字大小
2. 選出最小值（最便宜的店）
3. 計算差額（貴的 − 便宜的 ＝ 省了多少）
4. 排序三個數字由小至大（三商店困難模式）
...
|------|---------|---------|
| 兩店比較 | 30 組 | 5~800 元 |
| 三店排序 | 15 組（`B4_TRIPLE_ITEMS`）| 最小差 10 元 |

**商品類別**：日用品 / 零食飲料 / 文具 / 玩具 / 服飾

**防作答慣性設計**：
- 50% 機率左右交換兩店位置，避免學生慣性選「左側」
- 三商店模式：每題隨機打亂三店顯示順序
...
| 階段 | 學生動作 | 系統回饋 |
|------|---------|---------|
| 1. 選店 | 點擊便宜的那家 | 勾勾 + 比例橫條（紅/綠）|
| 2. 差額計算 | 選擇 / 輸入省多少元 | 差額算式（A店 − B店 ＝ ？）|
| 完成 | 自動下一題 | 省錢浮動 toast |
...
| 困難 | 依序點 1️⃣ 2️⃣ 3️⃣（價格隱藏）| 排序正確即得分 |

**困難模式自動提示**：10 秒未點擊 → 最便宜商品綠框脈動（`b4AutoHintPulse`）+ 語音「提示：先找最便宜的」

#### 難度遞進設計

| 維度 | 簡單 | 普通 | 困難 |
|------|------|------|------|
| 比較模式 | 選最便宜（無差額）| 選最便宜 + 差額選擇 | 完整排序（隱藏價格）|
| 差額題型 | — | 三選一 | 鍵盤輸入 |
| 差額提示 | — | 算式（答錯後）| 提示鈕（困難）|
| 三商店排序 | 選最便宜即完成 | 選最便宜 + 差額 | 依序點 1→2→3 |

#### 完成畫面特色資訊

| 區塊 | 內容 |
|------|------|
| 🏅 省錢排行榜 | top-3 最大省幅商品（金/銀/銅牌）|
| 🛒 比價歷程表 | 商品 / 便宜 / 較貴 / 省下 4 欄斑馬表 |
| 💰 累計節省橫幅 | 「你總共省了 X 元！」醒目黃色橫幅 |
| 省錢即時 toast | 每題答對後底部升起浮動提示 |

---

### 5.5 B5 生日派對預算 — 多商品限制預算決策

#### 教學定位

**學習目標**：在固定預算限制內，從多件商品中選擇搭配，使總金額不超過預算且包含所有必買項目，訓練「預算規劃與取捨決策」能力。

**前置知識**：多數加法（F6）→ 比較大小（F4）→ 貨幣概念（C1~C4）→ 規劃能力

**核心能力訓練**：
1. 累加多件商品金額，即時判斷是否超預算
2. 在超預算時決定「移除哪件商品」
3. 最大化選購（在預算內盡量多選）
4. 理解「必需品 vs 選擇品」的消費概念
...
| 類型 | 件數 | 範例 | 定價 |
|------|------|------|------|
| 必買（鎖定）| 2 件 | 🎂 蛋糕 + 🥤 飲料 | 380 + 120 ＝ 500 元（固定）|
| 可選（自由）| 10 件 | 氣球/蠟燭/裝飾/遊戲等 | 30~200 元 |

**32 個關卡情境**：每情境指定不同預算（600~1,200 元）與可選商品子集

#### 測驗方式（關卡制流程）

1. **開場介紹卡**（每關）：全屏卡顯示「第 N 關」+ 預算大字 + 必買商品，語音介紹，1.8 秒後或點擊關閉
2. **選購頁**：點擊可選商品卡片（toggle 選取 / 取消），必買商品灰底鎖定
3. **即時預算儀表條**：`[████████░░] 70%` 顏色同步變化（綠→橘→紅）
4. **確認按鈕 ready 脈動**：合法金額時按鈕顯示綠色光暈（`b5ConfirmPulse`）
5. **超支處理**：紅色超支提示 + 建議移除商品（貪婪演算法）+ 高亮脈動
6. **轉場卡**（非最後關）：紫色全屏卡「第 N 關完成！」1.1 秒後進下關

**提示機制**：
- 💡 提示鈕：高亮目前「還買得起」的可選商品 + 語音「還剩 X 元，可以加選 Y」
- 超支建議：自動計算「移除哪一件最接近預算」+ 2.4 秒後高亮該商品
...
|------|------|------|------|
| 關卡數 | 12 | 12 | 8 |
| 預算彈性 | 寬鬆（可選多件）| 中等壓力 | 緊繃（僅少數商品可選）|
| 💡 提示鈕 | 顯示 | 顯示 | 隱藏 |
| 超支建議 | 顯示 + 高亮 | 顯示 | 無建議 |
| 可選商品數 | 6~8 件 | 8~10 件 | 10~12 件（但預算緊）|

#### 完成畫面特色資訊

| 區塊 | 內容 |
|------|------|
| 🎉 本次派對採購 | 所有關卡選中商品去重展示（粉色泡泡標籤）|
| 📊 各關預算使用 | 橫條圖（ok 綠 / near 橘 / over 紅）|

---

### 5.6 B6 菜市場買菜 — 完整購物流程模擬

#### 教學定位

**學習目標**：完整模擬菜市場購物流程，訓練學生「看清單買菜 → 付款 → 找零確認」的完整金錢使用能力。

**前置知識**：認識紙鈔面額（C1/C3）→ 累加金額（C4）→ 找零計算（C6）→ 生活情境理解

**核心能力訓練**：
1. 閱讀採購清單，在多個攤位找到對應商品
2. 累加各商品價格計算總額
3. 選擇面額湊出足夠的付款金額
4. 計算找零（困難：三選一確認）
...
**攤位切換設計**：
- Tab 分頁切換，切換時語音播報攤位名稱
- 攤位徽章：待採購件數（紅色數字）→ 全收完（綠色 ✓）
- 容錯設計：點到不在清單的商品 → 錯誤 toast + 提示「這裡需要：X、Y」
...
| 動作 | 系統回饋 |
|------|---------|
| 點擊清單商品 | 商品彈跳動畫 + 浮動「+X 元」標籤 + 語音「商品名，X 元」|
| 攤位全部收集 | 攤位 Tab 徽章變綠 ✓ |
| 全部商品收集完畢 | 語音「所有商品收集完成，可以去結帳了！」|

**第二段：付款（Payment）**
| 動作 | 系統回饋 |
|------|---------|
| 點選面額按鈕 | 累計顯示已付金額 |
| 首次達到足額 | 語音「金額剛好！」/ 「超過 X 元，找零後可以付款！」|
| 點擊清除 | 語音「清除，重新選擇」|
| 💡 提示鈕 | 貪婪演算法高亮最少張數面額 + 語音說明 |

**第三段：找零（Change）**
| 難度 | 方式 | 回饋 |
|------|------|------|
| 簡單 / 普通 | 直接顯示找零金額 | 語音播報 |
| 困難 | 三選一問答（含偏移干擾項）| 答對綠框 / 答錯紅框 + 可繼續選 |
...
|------|------|------|------|
| 攤位數 | 1~2 | 2~3 | 3 |
| 商品件數 | 2~3 件 | 3~4 件 | 4~5 件 |
| 總金額 | 50~90 元 | 100~175 元 | 155~325 元 |
| 找零方式 | 直接顯示 | 直接顯示 | 三選一問答 |
| 付款提示鈕 | 顯示 | 顯示 | 隱藏 |
| 開場任務彈窗 | ✓ 2.8s | ✓ 2.8s | ✓ 2.8s |
| 攤位提示 | ✓ | ✓ | ✓ |
...
| 維度 | B1 | B2 | B3 | B4 | B5 | B6 |
|------|----|----|----|----|----|----|
| **核心運算** | 加法（湊總額）| 連續加減法 | 除法（無條件進位）| 比較 + 差額 | 加法 + 判斷 | 加法 + 找零 |
| **互動方式** | 拖曳錢幣 | 選擇 / 鍵盤 | 拖曳 / 選擇 / 鍵盤 | 點擊選擇 | 點擊 toggle | 點擊多步驟 |
| **難度標誌** | 費用隱蔽 | 事件數量 | 模式切換 | 排序深度 | 預算緊迫度 | 攤位商品數 |
| **輔助提示** | 任務彈窗 | 逐步算式 | 除法公式 | 差額算式 | 建議移除 | 面額提示 |
| **完成回顧** | 行程清單 | 日記回顧 | 目標清單 | 比價歷程 | 派對採購 | 採購收據 |
| **連勝徽章** | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| **語音深度** | 情境播報 | 計算提示 | 存錢引導 | 比價播報 | 預算提示 | 購物引導 |
| **作業單** | ✓（6 種題型）| ✓（2 種題型）| ✓（週數計算）| ✓（比價填表）| ✓（預算規劃）| ✓（購物清單）|

---

## 陸、2026-03-29 第三十輪豐富化總覽

> 本輪對齊 A4 店家類型、B5 主題、C4 面額篩選等「情境篩選器」模式，為 B 系列六個單元新增內容分類維度，顯著提高題庫多樣性與教學針對性。

### 6.1 新增功能一覽

| 功能 | 單元 | 實作模式 | 新增資料量 |
|------|------|---------|-----------|
| 場景類別篩選（school/food/outdoor/entertainment/shopping/all）| B1 | A4 storeType pattern | +39 組情境（原有全加 cat 欄位）|
| 日記主題（school/holiday/family）| B2 | B5 partyTheme pattern | +3 主題 × 3 難度 × 6-8 組 = 56 組模板 |
| 目標類別（toy/book/outdoor/tech/all）| B3 | B1 sceneCategory pattern | 現有 20 件加 cat 欄位 |
| 三商店排序 + 單位比價模式 | B4 | F4 排序 + F5 量比較 pattern | B4_TRIPLE_ITEMS 15 組、B4_UNIT_ITEMS 12 組 |
| 派對主題（birthday/halloween/picnic）| B5 | A4 storeType pattern | +2 主題 × 3 難度 × 6-8 組 = 40+ 場景 |
| 市場類型（traditional/supermarket/nightmarket）| B6 | A4 storeType pattern | +2 市場 × 3 難度 × 6-8 組 = 42 關卡 |
| B1~B6 輔助點擊模式（AssistClick）| B1~B6 | A 系列 AssistClick pattern | 新模組 + 設定選項 × 6 |
...
```
1. 資料層：在題庫物件加 cat/theme/marketType 欄位
2. 設定層：state.settings 加對應欄位（null = 未選）
3. UI 層：設定頁加 b-btn-group + data-xxx 按鈕
4. 事件層：_bindSettingsEvents 綁定，_checkCanStart 守衛
5. 篩選層：_generateQuestions / startGame 依設定篩選題庫
6. 顯示層：header 中心欄 / 完成畫面標題動態顯示當前情境
```

**fallback 機制**：若篩選後不足 2 題，自動退回完整題庫（B3 itemCat 適用）。

---

### 6.3 B2 日記主題詳析

| 主題 | 圖示 | Easy 組數 | Normal 組數 | Hard 組數 | 特色場景 |
|------|------|----------|------------|----------|---------|
| 學校週記（school）| 🏫 | 8 | 6 | 4 | 文具/便當/補習費/競賽獎勵 |
| 假期日記（holiday）| 🎉 | 8 | 6 | 4 | 過年紅包/遊樂園/生日禮金/暑期旅遊 |
| 家庭日記（family）| 👨‍👩‍👧 | 8 | 6 | 4 | 家務獎勵/採購幫手/家電/全家旅遊 |

> 三主題均覆蓋**收入型**（紅包/獎勵）與**支出型**（採購/活動）事件，確保每道題都有正數/負數混合練習。

---

### 6.4 B5 派對主題詳析

| 主題 | 圖示 | 必買商品 | 選購商品 | Easy 關卡 | Normal 關卡 | Hard 關卡 |
|------|------|---------|---------|----------|------------|----------|
| 生日派對（birthday）| 🎂 | 生日蛋糕、氣球 | 8 種 | 12 | 12 | 6 |
| 萬聖節派對（halloween）| 🎃 | 南瓜燈、萬聖節服裝 | 8 種 | 8 | 8 | 6 |
| 春日野餐（picnic）| 🌸 | 三明治、野餐墊 | 8 種 | 8 | 8 | 6 |

> `must:true` 商品以紫色「必買」徽章顯示，且 locked（不可取消）。每主題的商品價格分布確保不同預算關卡均有可行解。

---

### 6.5 B6 市場類型詳析

| 市場 | 圖示 | 攤位 1 | 攤位 2 | 攤位 3 | Easy | Normal | Hard |
|------|------|--------|--------|--------|------|--------|------|
| 傳統市場（traditional）| 🏪 | 蔬菜攤 | 水果攤 | 雜貨攤 | 8 關 | 8 關 | 6 關 |
| 超市（supermarket）| 🛒 | 烘焙區 | 乳品區 | 冷凍區 | 8 關 | 8 關 | 6 關 |
| 夜市（nightmarket）| 🏮 | 小吃攤 | 飲料攤 | 紀念品攤 | 8 關 | 8 關 | 6 關 |

**實作特點**：
- `_currentStalls` / `_currentMissions` 模組變數在 `startGame()` 時切換，所有原有邏輯（攤位 tab、商品點擊、付款、找零）**零修改**即可支援不同市場
- `activeStall` 改為 `Object.keys(_currentStalls)[0]`（不再 hardcode `'vegetable'`）
- `stallStats` 完成畫面攤位分析正確顯示各市場的攤位名稱

---

### 6.6 B4 比較模式擴充詳析

| 模式 | 資料集 | Easy | Normal | Hard | 特色 |
|------|-------|------|--------|------|------|
| 兩家店（two）| B4_ITEMS（30 組）| 點選便宜 | 差額三選一 | 輸入差額 | 基礎比較 |
| 三家店（triple）| B4_TRIPLE_ITEMS（15 組）| 點最便宜 | 選最便宜+差額三選一 | 依序 1️⃣2️⃣3️⃣排列 | F4 排序應用 |
| 單位比價（unit）| B4_UNIT_ITEMS（12 組）| 點每單位便宜 | 單位差額三選一 | 輸入每單位差額 | 除法應用（÷）|

> **單位比價**是 B4 最高難度的延伸模式，要求學生計算「每個 X 元」而非「總價 X 元」，對應 F5 量比較的高階概念。

---

### 6.7 更新後的 B 系列設計矩陣（2026-03-29）

| 維度 | B1 | B2 | B3 | B4 | B5 | B6 |
|------|----|----|----|----|----|----|
| **核心運算** | 加法（湊總額）| 連續加減法 | 除法（無條件進位）| 比較 + 差額（含單位除法）| 加法 + 判斷 | 加法 + 找零 |
| **情境篩選** | 🗂️ 場景類別（6）| 📓 日記主題（3）| 🗂️ 目標類別（5）| 🏬 比較模式（3）| 🎪 派對主題（3）| 🏪 市場類型（3）|
| **互動方式** | 拖曳錢幣 | 選擇 / 鍵盤 | 拖曳 / 選擇 / 鍵盤 | 點擊選擇（含排序）| 點擊 toggle | 點擊多步驟 |
| **難度標誌** | 費用隱蔽 | 事件數量 | 模式切換 | 排序深度 + 單位 | 預算緊迫度 | 攤位商品數 |
| **輔助提示** | 任務彈窗 + 費用明細 | 逐步算式 + 開場彈窗 | 除法公式 + 週數方塊 | 差額算式 + 排序提示 | 建議移除 + 預算儀表 | 面額提示 + 錯誤攤位提示 |
| **完成回顧** | 行程清單 + 面額統計 | 日記回顧 + 收支總計 | 目標清單 + 存錢摘要 | 比價歷程 + 省錢排行 | 派對採購 + 各關預算 | 採購收據 + 攤位分析 |
| **連勝徽章** | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ |
| **輔助點擊** | ✓（貪婪面額）| ✓（逐位數）| ✓（hard quiz）| ✓（select+diff+排序）| ✓（高亮可負擔）| ✓（購物+付款+找零）|
| **語音深度** | 情境播報 | 計算提示 | 存錢引導 | 比價播報 | 預算提示 | 購物引導 |
| **作業單** | ✓（6 種題型）| ✓（2 種題型）| ✓（週數計算）| ✓（比價填表）| ✓（預算規劃）| ✓（購物清單）|

---

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-29*

---

## 柒、2026-03-29 第二十三輪豐富化總覽

### 7.1 新增功能一覽

| 功能 | 單元 | 說明 |
|------|------|------|
| B4 商品類別篩選 | B4 | food/stationery/daily/clothing 四類別 + 全部預設；三個題庫均加 `cat` 欄位 |
| B4 開題商品介紹彈窗 | B4 | 每題顯示商品 icon + 兩店價格對比，1800ms 後消失 |
| B5 超支費用明細展開 | B5 | 答錯時展開所有已選商品的費用明細表，超出金額含百分比 |
| B6 找零算式提示 | B6 | change quiz 答錯後顯示「付X元 − 商品Y元 = 找零Z元」公式 |
| B4 節省百分比 | B4 | savings toast 改為「省了 X 元（省 Y%）」，教導百分比概念 |
| B2 收支類型徽章 | B2 | 每條日記事件顯示「收入 📥」/「支出 📤」綠/紅膠囊徽章 |
| B6 第一關進場語音 | B6 | 第一關顯示市場名稱，語音加「歡迎來到{市場名}！」前綴 |

### 7.2 B4 商品類別篩選設計

B4 已有「比較方式」設定（兩家店/三家店/單位），第二十三輪補充「商品類別」設定（預設全部，非強制）：

| 類別 key | 中文標籤 | 代表商品 |
|---------|---------|---------|
| `all` | 全部 | 所有 40 組題目 |
| `food` | 食品飲料 | 蘋果、礦泉水、巧克力、牛奶、餅乾等 |
| `stationery` | 文具書籍 | 鉛筆盒、原子筆、故事書、色鉛筆等 |
| `daily` | 生活用品 | 洗髮精、毛巾、牙刷、電池、面紙等 |
| `clothing` | 服飾配件 | 雨傘、運動鞋、拖鞋、手套、帽子等 |

**篩選邏輯**：`B4_ITEMS.filter(i => i.cat === itemCat)`；若篩選後 < 2 項則 fallback 至全部（防空集合）。B3 `itemCat` pattern。

### 7.3 B5 費用明細設計

超支時在原有「超出建議」下方展開費用明細：
...
這讓學生清楚看到是哪個商品導致超支，對應 B2 `_showCalcBreakdown` + B1 `_showScheduleBreakdown` 的教學邏輯。

### 7.4 更新後的 B 系列設計矩陣（2026-03-29 第二十三輪）

| 維度 | B1 | B2 | B3 | B4 | B5 | B6 |
|------|----|----|----|----|----|----|
| **情境篩選** | 🗂️ 場景（6）| 📓 日記主題（4+random）| 🗂️ 目標類別（5）| 🏷️ 商品類別（5）+比較方式（3）| 🎪 派對主題（4+random）| 🏪 市場類型（4+random）|
| **開題互動** | 任務說明彈窗 | 起始金額彈窗 | 存錢目標彈窗 | 商品對比彈窗 | 關卡預算卡 | 關卡任務彈窗 |
| **答錯教學** | 費用明細展開 | 逐步算式 + 評分 | 除法算式提示 | 差額公式 + 類別提示 | 超支費用明細 + % | 找零公式提示 |
| **百分比教育** | — | — | — | ✓ 節省 X% | ✓ 超出 X% | — |
| **類型分類** | 5 種場景類別 | 收入/支出 徽章 | 4 種商品類別 | 4 種商品類別 | 必買/選購 標記 | 3 種攤位類別 |
...
## 捌、2026-03-29 第二十四輪豐富化總覽

### 8.1 新增功能一覽

| 功能 | 單元 | 說明 |
|------|------|------|
| B4 三商店獎台動畫 | B4 | 排序正確後顯示 2nd/1st(gold)/3rd 三格獎台，教學視覺強化 |
| B5 節省金額徽章 | B5 | 成功時若 `rem > 0`，顯示「💰 節省了X元（節省Y%）！」綠色徽章 |
| B6 結帳確認清單 | B6 | 點擊結帳前先彈出商品清單+合計+預算確認卡，語音播報合計金額 |
| B3 月曆倒數提示 | B3 | 每次存入後底部浮動「還差X元，再存Y天就達標！」倒數提示 |

### 8.2 B4 三商店獎台動畫設計

排序正確後，`_showPodiumAnimation(curr)` 在畫面下方顯示三格獎台（`b4-podium-overlay`）：
...
每欄 `b4PodiumRise` 動畫依序出現（`animation-delay: 0 / 0.12s / 0.24s`），1.6s 後淡出。F4 排序視覺 + C5 差額圖示 pattern。
...
_showCheckoutConfirm(g, callback)
    ├─ 顯示 .b6-checkout-card（b6CcIn 彈出動畫）
    │   ├─ 商品清單（name + price）
    │   ├─ 合計金額（藍色加粗）
    │   └─ 預算金額（灰色）
    ├─ 語音：「合計X元，預算Y元，確認去付款！」
    ...
    └─ 5s 自動 → callback()
        ↓
    callback()：進入 payment phase
```

AssistClick 同步更新：當 `#b6-cc-go` 存在時優先高亮它而非 `b6-checkout-btn`。

### 8.4 B3 月曆倒數提示設計

每次拖曳存錢完成後（非達標情況），新增：
1. **語音**：「存入X元！還差Y元，再存Z天就達標了！」
2. **底部浮動卡**（`_showCountdownHint`）：
   ```
   ┌─────────────────────────┐
   │  [200] 元・再存 5 天    │  ← b3-cd-num + b3-cd-label
   └─────────────────────────┘
   ```
   - 位置：`bottom: 24px`，水平置中
   - 動畫：`b3CdIn`（從下方滑入）
   - 持續：2s 後淡出
   - 天數計算：`Math.ceil(remaining / c.dailyAmount)`

### 8.5 B5 節省金額徽章設計

`_handleConfirm` 成功路徑：
```js
const remPct = Math.round((rem / g.budget) * 100);
// rem > 0 時顯示：
💰 節省了 50 元（節省 7%）！
```

綠色漸層徽章（`#d1fae5 → #a7f3d0`），`bFadeIn` 動畫，出現在 success banner 下方。

教學目的：讓學生意識到在預算內完成任務的同時，節省的金額也有意義（預算效率概念）。
...
## 玖、2026-03-29 第二十五輪豐富化總覽

### 9.1 新增功能一覽

| 功能 | 單元 | 說明 |
|------|------|------|
| B5 預算效率徽章 | B5 | 答對後顯示預算使用率評級（💎完美/⭐善用/👍不錯/💡節省） |
| B6 攤位小計提示 | B6 | 切換攤位時顯示上一攤位已花金額（累計教學） |
| B1 找零說明動畫 | B1 | 答對且多付時，顯示「付X−需Y＝找回Z元」公式卡 |
...
教學目標：讓學生理解「花最多預算 ≠ 浪費」，在預算內最大化購買是派對規劃的核心技能。
...
- 位置：底部固定，水平置中
- 觸發：`diff > 0` 時，答對後 300ms 延遲出現
- 持續：2.2s 後淡出
- 教學目的：每次多付款時，學生能即時看到找零算式，強化「付款 − 費用 = 找零」概念
...
## 拾、2026-03-29 第二十六輪豐富化總覽

### 10.1 新增功能一覽

| 功能 | 單元 | 說明 |
|------|------|------|
| B2 收支趨勢指示 | B2 | 答對後頂部浮現盈餘↑（綠）或赤字↓（紅）指示 |
| B3 月曆距完成天數 | B3 | info card 動態顯示「距完成N天」，≤3天時脈動提示 |
| B5 完美配額特效 | B5 | `rem===0` 時觸發完美特效（💯 + 黃色 banner + 粉色 eff-badge） |

### 10.2 B2 收支趨勢設計

```
答對 → _showNetTrend(question)
         net = answer - startAmount
         net > 0 → "↑ 本週盈餘 +X元"  （綠色，b2NtIn 滑入）
         net < 0 → "↓ 本週赤字 -X元"  （紅色）
         net = 0 → 不顯示（平衡）
```

教學目標：讓學生直觀理解「收入 > 支出 → 盈餘」「支出 > 收入 → 赤字」的財務概念。

### 10.3 B5 完美配額比較

| 狀態 | 顯示 | 語音 |
|------|------|------|
| `rem > 0` | 🎉 太棒了 + savings badge | 「共花了X元，還剩Y元」 |
| `rem === 0` | 🎯 完美配額！（黃色 banner）| 「完美！剛好花了X元，用完全部預算！」 |
...
## 拾壹、2026-03-29 第二十七輪豐富化總覽

### 11.1 新增功能一覽

| 功能 | 單元 | 說明 |
|------|------|------|
| B4 較貴商品差額標籤 | B4 | select 答對時，較貴的卡片顯示「比較貴 +N元」紅色標籤 |
| B6 收集進度動畫 | B6 | 每次收集商品後右側顯示「+1 X/Y」浮動計數器 |
| B2 易模式週收支統計 | B2 | easy 動畫接近結束時插入「📥 收入X元 ｜ 📤 支出Y元」統計 |

### 11.2 B4 差額標籤設計

兩商店 select 正確時：
- 正確卡（較便宜）：`.b4-cheaper-tag` 綠色標籤「比較便宜！」（原有）
- 錯誤卡（較貴）：`.b4-exp-delta` **新增**紅色標籤「比較貴 +N元」

`delta = |optA.price - optB.price|`，讓學生一眼看到兩店的價格差距。unit mode 不顯示（因單位不同，差距的意義不同）。

### 11.3 B6 收集進度設計

```
收集商品 → _showCollectionProgress(collected, needed)
    → 右側浮動卡（b6CpIn 彈出）
    ┌──────┐
    │  +1  │  ← b6-cp-plus（綠色）
    │ 2/3  │  ← b6-cp-count
...
entries 動畫時間軸：
  ...
  1300ms → 第2個
  2100ms → 第3個 ...
  (500 + n*800)ms → 最後 entry
  (500 + n*800 + 100)ms → 插入週收支統計（b2-week-summary）
  (500 + n*800 + 500)ms → 移除統計，顯示 answerArea
```
...
## 拾貳、2026-03-29 第二十八～三十二輪豐富化總覽

### 12.1 第二十八輪（已於上一session提交）
| 功能 | 單元 |
|------|------|
| B6 精準付款特效 | B6 |
| B3 平均每週存款統計 | B3 |
...
|------|------|
| B1 錢包進度條（藍→橙→綠） | B1 |
| B5 各關總計摘要（總預算/總花費/節省/平均使用率） | B5 |

### 12.3 第三十輪：視覺豐富化
| 功能 | 單元 | 設計說明 |
|------|------|---------|
| 今日路線條 | B1 | `_renderScheduleCard` 末尾顯示「🏠 → icon label → 🏠」，動畫 0.3s 延遲 |
| 餘額走勢條 | B2 | easy 動畫後顯示 `endBalance/startAmount` 百分比橫條（good/ok/low 三色）|
| 進度環 | B3 | 撲滿卡片加 `conic-gradient` 圓形進度環，`_updateCalendarUI` 同步 deg |
| 超限震動 | B5 | `_updateTotalBar` 首次超出時 `b5Shake` 600ms 動畫 |
| 浮動購物籃徽章 | B6 | 收集商品後右下角固定徽章顯示「X/Y」計數 |

### 12.4 第三十一輪：互動反饋
| 功能 | 單元 |
|------|------|
| 即時餘額預覽（numpad 輸入時同步顯示 exact/over/under）| B2 |
| 最佳存法提示（答錯後顯示半量/原量/雙量對比）| B3 |
| 冠軍徽章（two-store select 答對中央「🥇X最便宜！」）| B4 |
| 預算效率星評（結果頁 1-3 星，依 avgPct 決定）| B5 |

### 12.5 第三十二輪：教育強化
| 功能 | 單元 |
|------|------|
| 面額計數摘要（錢包中各面額 N元×M 統計）| B1 |
| 理財建議卡（答對後底部 toast，依盈虧顯示建議）| B2 |
| 減法學習要點（diff 答對後「X − Y = Z元」公式 toast）| B4 |
| 結帳平均金額（確認清單新增「共N項，平均每項M元」）| B6 |

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-29*

---

## 拾參、2026-03-29 第三十三～三十五輪豐富化（參照 A/C/F 系列強化）

### 13.1 第三十三輪：感知回饋強化（參照 A4 動畫、C5 錯誤差異）

| 功能 | 單元 | 說明 |
|------|------|------|
| 差額錯誤語音 | B1 | 「還差X元，再多加一些！」精確說出差距 |
| 答題動畫 | B2 | 正確→綠光暈 / 錯誤→抖動（`b2AnswerGlow/b2AnswerShake`）|
| numpad 錯誤辨識語音 | B2 | 「算太多了，多了X元」/ 「還有X元沒算到」|
| 正確卡片光暈 | B4 | 選對店後 `b4CardGlow` 動畫 |
| 勳章制（全系列）| B1~B6 | 100%→🥇完美；≥90%→🥇；≥70%→🥈；≥50%→🥉；其他→⭐ |
| 付款效率環形圖 | B6 | SVG 圓弧圖顯示精準付款比例 |

### 13.2 第三十四輪：難度漸進與教學鷹架（參照 A4/F5 漸進提示系統）

| 功能 | 單元 | 設計說明 |
|------|------|---------|
| 漸進提示 | B2 | numpad 第1次錯→範圍提示（lo~hi），第2次→完整算式 |
| 配速預覽 | B3 | easy 模式每個選項顯示「每週X元 × Y週 = Z元」算式 |
| 預算分配說明 | B5 | intro card 顯示「必買X元 + 選購Y元」 |
| 找零兩段提示 | B6 | 第1次錯→公式架構；第2次→完整找零算式 |

### 13.3 第三十五輪：視覺敘事與難度差異化（參照 F5 量比較、A4 scaffolding）

| 功能 | 單元 | 設計說明 |
|------|------|---------|
| 入場動畫 | B1 | 行程項目 140ms stagger slide-in |
| 起始/結束對比條 | B2 | easy 動畫後顯示雙橫條比較始末金額 |
| 里程碑語音 | B3 | 25/50/75% 里程碑加音效 + 語音慶祝 |
| 最佳比價摘要 | B4 | 結果頁顯示本次最划算的商品與節省金額 |
| 困難隱藏價格 | B5 | hard mode 非必買商品顯示 "??? 元"，點擊揭示後再點才能選取 |

### 13.4 A/C/F → B 系列 Cross-Pattern 對照

| A/C/F 特色 | 實作到 B 系列 |
|-----------|-------------|
| A4 gradated hint (1/2/3 error escalation) | B2 範圍→算式漸進；B6 架構→公式漸進 |
| A4/C5 error-specific speech | B1 差額語音；B2 多/少辨識語音 |
| A4/F5 glow & shake animations | B2 答題動畫；B4 卡片光暈；B5 超限震動 |
| A4/C5 medal tiers (Gold/Silver/Bronze) | B1~B6 勳章制全面升級 |
| F5 scaffolding (hide info by difficulty) | B5 困難隱藏價格（首次點擊揭示） |
| B3 milestone badge → speech | B3 里程碑加語音（Round 35）|
| F5 comparison bars | B2 起始/結束對比條 |

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-29*

---

## 拾肆、2026-03-29 第三十六～三十七輪豐富化（參照 A/C/F 系列強化）

### 14.1 第三十六輪：即時狀態反饋（參照 A6 確認脈動、B6 攤位完成、F5 量比較）

| 功能 | 單元 | 說明 |
|------|------|------|
| 投幣足額彈出動畫 | B1 | `_updateWalletDisplay` 偵測 `enough && !wasEnough`，觸發 `b1-total-pop` 彈出動畫（500ms 後移除），對齊 A6 確認付款脈動 pattern |
| 差額百分比標示 | B4 | `_renderPriceBars` 在雙欄比例條右側加 `便宜了 X%` 橙色標籤（`b4-pbar-diff-pct`），強化百分比概念 |
| 必買門檻標線 | B5 | `_updateTotalBar` 計算 `mustPct`，在進度條上顯示琥珀色垂直標線 `b5-must-marker`，幫助學生區分必買/選購 |
| 攤位完成閃光 | B6 | 攤位商品全收完時，tab 加 `b6-stall-done-flash` class（800ms 後移除），對齊 A6/C5 狀態完成動畫 |

### 14.2 第三十七輪：語音強化與語境提示（參照 A4 即時語音、B1 組合提示）

| 功能 | 單元 | 說明 |
|------|------|------|
| 建議硬幣組合卡 | B1 | `_showCoinHint` 改為 DOM 卡片 `#b1-hint-combo-card`：顯示建議面額組合，5 秒後自動消失，對齊 A4 任務說明卡 pattern |
| Easy 模式圖例 | B2 | `_renderQuestionHTML` easy 模式加 `b2-legend` 行說明收入(+)/支出(-) 符號意義 |
| 月曆預估達標日 | B3 | `_updateCalendarUI` 計算每日節奏推估完成日期，`#b3-est-date` 顯示「預計 M/D 達標」/ 已達標時顯示「🎉 達標！」 |
| 選對即播報價格 | B4 | `handleSelectClick` 正確分支即時語音「X店，Y元，比較便宜！」，強化數字-語音連結 |
| 商品飛出收據 | B6 | `_showItemReceiptFlyout(anchor, item)` 在收集商品時彈出 `icon + name + +N元` 浮動標籤，從商品位置飛出消失（`b6FlyoutUp`）|

### 14.3 A/C/F → B 系列 Cross-Pattern 對照（第三十六～三十七輪）

| A/C/F 特色 | 實作到 B 系列 |
|-----------|-------------|
| A6 確認付款脈動 (`ready` class) | B1 `b1-total-pop` 足額彈出；B5 `b5-must-marker` 門檻標線 |
| A4 即時語音（`speakCustom` 選對後即說）| B4 選對即播報「X店最便宜」語音 |
| B1 `_showCoinHint` 改版 → DOM 卡片 | B1 `#b1-hint-combo-card` 5秒浮動卡 |
| F5 進度條 + 百分比 | B4 差額百分比標示；B5 mustMarker 門檻標線 |
| A4 收據飛出動畫 | B6 `b6-item-flyout` 商品收集飛出效果 |
| C2 逐項學習說明 | B2 easy legend 收支符號說明 |

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-29*

---

## 拾伍、2026-03-29 第三十八輪豐富化（互動引導強化）

### 15.1 第三十八輪：感知層強化（參照 A4 比例視覺、F2 粒子動畫、A4 記憶鷹架）

| 功能 | 單元 | 說明 | 參照模式 |
|------|------|------|---------|
| 費用佔比條 | B1 | 每個行程費用下顯示佔總費用比例的橙色橫條，困難模式不顯示 | F5 量比較橫條 |
| 事件累計金額列 | B2 | 每條收支事件後顯示當下餘額（灰色小字），幫助學生追蹤金錢變化流程 | A6 找零逐步展示 |
| 存錢粒子特效 | B3 | 月曆存錢成功後從撲滿位置噴出 5 顆 ✨💫⭐🌟💰 粒子飛散動畫 | F2 星星動畫 pattern |
| 困難記憶倒數 | B4 | 困難模式題目顯示 1.9 秒後啟動 3 秒倒數，倒數結束模糊所有價格，考驗記憶力 | A4/C1 scaffolding pattern |
| 可負擔商品高亮 | B5 | `_updateTotalBar` 後對所有「未選且買得起」的選購商品加虛線綠框，幫助學生決策 | C5 狀態指示燈 pattern |
| 全部收集完成閃光 | B6 | 所有商品首次收集完成時顯示中央「🎉 全部收集完成！」綠色閃光卡（1.5 秒）| A6 確認動畫 pattern |

### 15.2 教育設計說明

- **B1 費用佔比**：比例視覺強化學生的「整體-部分」概念，為分數/百分比做鋪墊
- **B2 累計金額**：每步顯示即時餘額，實現「邊讀邊算」的認知輔助
- **B3 粒子特效**：即時正面強化存錢行為，提升動機和成就感
- **B4 記憶倒數**：困難模式增加「記憶+計算」雙重挑戰，對齊 A4/A1 高難度設計
- **B5 可負擔高亮**：減輕認知負擔，引導注意力到「可行選項」
- **B6 完成閃光**：明確的完成感，激勵學生確認任務完成再結帳

### 15.3 A/C/F → B 系列 Cross-Pattern 對照（第三十八輪）

| A/C/F 特色 | 實作到 B 系列 |
|-----------|-------------|
| F5 量比較橫條（寬度比例）| B1 費用佔比條（每項費用佔總費用百分比）|
| A6 找零逐步展示 | B2 事件累計金額列（即時餘額追蹤）|
| F2 閃爍動畫粒子 | B3 存錢粒子（撲滿噴出 5 顆粒子）|
| A4/C1 記憶鷹架（難度層疊）| B4 困難記憶倒數（3 秒後模糊價格）|
| C5 指示燈可負擔狀態 | B5 可負擔商品綠框高亮 |
| A6 確認完成動畫 | B6 全部收集完成中央閃光卡 |

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-03-29*

---

## 拾陸、2026-03-29 第三十九輪豐富化（情境脈絡強化）

### 16.1 第三十九輪：語境與視覺脈絡強化

| 功能 | 單元 | 說明 | 參照模式 |
|------|------|------|---------|
| 場景類別色標 | B1 | schedule card 頂部 4px 色條（學校藍/飲食橙/戶外綠/娛樂紫/購物粉）依題目 cat 欄位決定 | A4 商店主題色 |
| 主題情境語音 | B2 | 語音問題前加主題前綴「學校週記，」/ 「假日時光日記，」/ 「家庭生活日記，」，random 模式不加 | B5 場景語音 pattern |
| 即時剩餘金額標籤 | B3 | `_updateWeekPreview` 顯示選擇週數後「還差X元」或「🎉 足夠！」，即時讓學生看到存款進展 | C5 即時反饋 |
| 累計節省徽章 | B4 | 答題過程中 header 顯示「💰 已省X元」累計節省綠色小徽章，激勵繼續比價 | A4 交易摘要徽章 |
| 即時選擇計數 | B5 | total-bar 新增「必買N件+選購M件」即時計數，幫助學生追蹤選擇狀況 | C5 指示燈 pattern |
| 攤位商品語音引導 | B6 | 切換攤位時改語音為「X攤，要找Y和Z」，明確說出需要收集的商品 | A4 任務語音 pattern |
...
## 拾柒、2026-03-30 系列技術指標更新（Rounds 26-39 後）

### 17.1 JS 規模成長對照表

| 單元 | 建立時（2026-03-14）| Rounds 1-20（2026-03-21）| Rounds 26-39（2026-03-30）| 成長倍率 |
|------|---------------------|--------------------------|--------------------------|---------|
| B1 | ~300 行 | ~1,036 行 | **1,516 行** | 5.1× |
| B2 | ~280 行 | ~907 行 | **1,669 行** | 6.0× |
| B3 | ~500 行 | ~835 行（初版）→ 1,400（月曆）| **2,863 行** | 5.7× |
| B4 | ~350 行 | ~867 行 | **2,078 行** | 5.9× |
| B5 | ~350 行 | ~781 行 | **1,403 行** | 4.0× |
| B6 | ~400 行 | ~980 行 | **1,997 行** | 5.0× |
| **合計** | ~2,180 行 | **~5,406 行** | **~11,526 行** | **5.3×** |

### 17.2 CSS 規模成長對照表

| 單元 | 建立時 | Rounds 1-20 | Rounds 26-39 | 成長倍率 |
|------|--------|-------------|--------------|---------|
| B1 | ~150 行 | ~758 行 | **1,029 行** | 6.9× |
| B2 | ~80 行 | ~276 行 | **737 行** | 9.2× |
| B3 | ~120 行 | ~650 行 | **1,729 行** | 14.4× |
| B4 | ~100 行 | ~296 行 | **826 行** | 8.3× |
| B5 | ~80 行 | ~243 行 | **687 行** | 8.6× |
| B6 | ~120 行 | ~375 行 | **967 行** | 8.1× |
| **合計** | ~650 行 | **~2,598 行** | **~5,975 行** | **9.2×** |

### 17.3 功能特色總量

| 功能類別 | B 系列累積數量 |
|---------|-------------|
| 設定選項維度 | 每單元 5 項（難度/題數/重試/主題篩選/輔助點擊）|
| 動畫 keyframe 數 | 共 ~80+ 個跨 6 個 CSS 檔案 |
| 完成畫面統計模組 | 每單元 3-5 個獨立統計卡片 |
| 開題彈窗設計 | B1~B6 全部具備 |
| AssistClick 模式 | B1~B6 全部具備 |
| 題庫主題篩選 | B1（場景）/ B2（日記）/ B3（目標）/ B4（商品）/ B5（派對）/ B6（市場）|
| 語音反饋節點 | 每單元 8-15 個觸發場景 |

### 17.4 B 系列 vs A/C/F 系列技術深度比較（2026-03-30 現況）

| 維度 | A 系列 | C 系列 | F 系列 | B 系列 |
|------|--------|--------|--------|--------|
| 平均 JS 行數 | ~4,000 行（A1最大~6,000）| ~3,200 行 | ~2,800 行 | **~1,920 行**（B3 最大 2,863）|
| 輔助點擊模式 | ✅（A1~A6）| ✅（C1~C6）| ✅（F1~F6）| ✅（B1~B6）|
| 完成畫面統計 | ✅ | ✅ | ✅ | ✅ |
| 作業單系統 | ✅ | ✅ | ✅ | ✅ |
| 主題/類別篩選 | ✅（A4 商店類型）| ⚪（無主題）| ⚪（無主題）| ✅（B1~B6 各有主題）|
| coinFirst 模式 | ✅（A1/A2）| — | — | —（B6 類似三段式）|
| ModeConfig 架構 | ✅（A4/A5/A6）| ✅（C1/C4）| ✅（F4/F5）| 部分（B3 雙模式）|

> **結論**：B 系列已達到功能豐富度中等偏高水準（接近 C/F 系列深度），但平均 JS 體量仍低於 A 系列（A 系列含 coinFirst、大量語音分支、自訂商品等複雜系統）。

---

## 拾捌、各單元教學特色精選（B vs A/C/F 最具代表性差異）

### 18.1 B1 — 情境導向錢幣組合（唯一拖曳型預算練習）

- **跨系列獨特點**：C/F 系列錢幣互動（C1 辨識、C4 湊出金額）均為「指定金額」型，B1 是唯一「先計算行程費用再準備錢」的兩步驟設計
- **場景色標（Round 39）**：從視覺上將不同生活場景（學校/飲食/戶外/娛樂/購物）用色條區隔，形成學習地圖感
- **費用佔比條（Round 38）**：每個費用項目下的比例條，讓學生直觀看到「這筆支出佔了多大比例」——跨系列中罕見的費用分析視覺化

### 18.2 B2 — 收支時序追蹤（日記型記帳教育）

- **跨系列獨特點**：B2 是唯一「時序型」題目——事件有先後順序，每筆收支對下一步有影響，強調時間維度
- **漸進提示（Round 34）**：第 1 次答錯給範圍提示，第 2 次才給完整算式，設計哲學對齊 C3/C6 兩段提示模式
- **收支徽章（Round 29）**：📥/📤 類型標示，呼應 C5 「夠不夠」的視覺判斷設計

### 18.3 B3 — 目標導向儲蓄（唯一含實體月曆的單元）

- **跨系列獨特點**：B3 easy 模式有獨一無二的「月曆拖曳存錢」體驗，不是任何 A/C/F 單元的翻版
- **進度環（Round 30）**：conic-gradient 圓弧進度環，呼應撲滿視覺，同時量化存錢進度
- **最佳存法提示（Round 31）**：同時顯示三種存法的對比，讓學生理解「存太少」「剛好」「存太多」的數學意義

### 18.4 B4 — 多場景比價（最複雜的 B 系列比較邏輯）

- **跨系列獨特點**：三商店排序模式（Round 25）讓 B4 超越一般「哪個便宜」，變成排序問題（類 F4），是 B/F 系列概念的高階融合
- **記憶倒數（Round 38）**：困難模式隱藏價格後靠記憶回答，是跨系列中強度最高的認知負荷設計
- **獎台動畫（Round 29）**：三商店排對正確後顯示 2nd/1st/3rd 獎台，教學趣味感爆棚

### 18.5 B5 — 限制條件下最優選（最貼近真實購物的預算練習）

- **跨系列獨特點**：必買 + 選購雙層結構，是 B 系列中最接近「真實生活預算決策」的設計
- **困難模式隱藏價格（Round 35）**：首次點擊才揭示，第二次才選取——模擬真實店裡翻看價格標籤的行為
- **預算效率徽章（Round 29）**：💎完美/⭐善用/👍不錯/💡節省 四級評定，不只評對錯，還評效率

### 18.6 B6 — 完整市場交易循環（最接近 A 系列深度的 B 單元）

- **跨系列獨特點**：B6 是唯一涵蓋「採購 → 付款 → 找零」完整交易流程的 B 單元，接近 A 系列複雜度
- **三市場類型（Round 26）**：傳統市場/超市/夜市，每種有不同攤位設定，是 A4「12 種商店」設計哲學的 B 系列版本
- **收據飛出動畫（Round 37）**：商品飛出小收據，呼應 A3/A4 交易摘要的「視覺購物滿足感」設計
...
## 二十一、B4 特賣比一比 — 介面重構紀錄（2026-04-10）

> **更新日期**：2026-04-10（Rounds 45–46：B4 全面互動重設計）

### 21.1 Round 45 — 版面與互動基礎重構

#### 21.1.1 紙鈔尺寸統一
- `b4PriceCoins` 紙鈔從 `height:auto` 改 `height:48.91px;object-fit:cover`
- 確保 100/500/1000 元紙鈔顯示高度一致

#### 21.1.2 省錢清單（第三頁）版面卡片化
- `showSavingsList` 改用 `.b-header`（`b4-sl-reward-btn`/`b4-sl-back-btn`）
- 新容器 `.b4-sl-page`；摘要橫條 `.b4-sl-summary-bar`
- 卡片網格 `.b4-sl-cards2`（`b4-sl-card2`：頂部分類色帶＋算式列）
- Summary 區改為 `.b4-sl-summary-card`（綠色漸層卡）含 trophy + 標題 + 總省金額 + 最划算

#### 21.1.3 簡單模式逐枚點幣後自動比較
- `_setupEasyModeCoins`：各幣可點，累計播金額，兩側全點完 → `_handleEasyBothSidesDone` 自動高亮便宜卡
- `_bindSelectEvents`：`diff === 'easy'` 不可直接點卡片

#### 21.1.4 普通模式差額頁版面重構
- `_renderDiffSection` normal：移除 `barsHTML`，算式卡 `.b4-diff-normal-card`，問題移至算式下方 `.b4-diff-question-below`，選項獨立卡片 `.b4-diff-options-card`

---

### 21.2 Round 46 — 全模式互動升級

#### 21.2.1 商品彈窗簡化
- `_showItemIntroModal` 移除 storesHTML/questionText，只顯示 icon + name
- 關閉後語音格式：`${store1}${price1}元，${store2}${price2}元，請問哪一個商店賣的比較便宜？`

#### 21.2.2 點幣語音完整播放保證
- `_setupEasyModeCoins` / `_setupNormalModeCoins` 最後一枚改用 `Game.Speech.speak(callback)` 確認語音結束後才觸發下一步
- 修正左右同時完成時可能發生語音截斷的問題

#### 21.2.3 普通模式第一頁點幣 → 輸入較便宜價格
- `_setupNormalModeCoins`：同 easy 點幣模式，兩側完成後呼叫 `_showPriceInputSection`
- `_showPriceInputSection`：顯示可點擊的 display box → 點後彈出浮動 modal 數字鍵盤
- 正確：reveal 卡片 + 進入差額頁；錯誤：播錯誤語音重試

#### 21.2.4 困難模式第一頁語音後輸入價格
- 價格以「？？？」灰字顯示（`.b4-price-unknown`），金幣保持隱藏
- 語音播完後 `_showPriceInputSection` 顯示輸入框
- 提示鈕按下：揭露價格 + 金幣 + 高亮正確卡（`_revealCardPrices` + `_revealCoinsOnly`）
- `_bindSelectEvents`：hard 模式僅困難可點卡片（normal 也改為不可直接點）

#### 21.2.5 簡單模式差額頁（第二頁）重構
- 移除 barsHTML，改用 formulaHTML + `.b4-diff-normal-card`
- 單一答案按鈕（`.b4-diff-opt-solo`）於 `.b4-diff-options-card`
- 進入語音：`${商品名}，${A店}${A價}元，${B店}${B價}元，請選擇正確的答案`

#### 21.2.6 普通模式差額頁（第二頁）選項遮罩
- 選項按鈕加 `b4-diff-opt-masked`，顯示「？？？」
- 點擊或提示鈕觸發 `_revealNormalDiffOptions` 揭露所有金額
- 提示鈕額外高亮正確選項（`b4-select-hint` 2.4s）
- 進入語音：`${商品名}，${貴店}${貴價}元，${便宜店}${便宜價}元，${便宜店}便宜了${差額}元，請選擇正確的答案`

#### 21.2.7 困難模式差額頁（第二頁）— 點？？？開彈窗
- 移除 question label + 內嵌 numpad/display
- `.b4-dff-unknown` 加 `cursor:pointer`，點擊 → `_showHardDiffNumpadModal` 彈窗
- 提示鈕 → `_showHardDiffFormulaHint`：彈窗顯示「💡 算式提示：A − B = C 元」並播語音
- 確認正確後更新算式格為實際數字（移除 `b4-dff-unknown` class）

#### 21.2.8 ref card 置中
- CSS：`.b4-diff-ref-cheap { text-align: center }`、`.b4-ref-card-wrap { align-items: center }`

---

### 21.3 新增 CSS 類別摘要（Round 45–46）

| 類別 | 說明 |
|------|------|
| `.b4-price-unknown` | ？？？ 灰字顯示（困難模式第一頁） |
| `.b4-diff-opt-masked` | 選項金額遮罩（普通模式） |
| `.b4-pi-card` / `.b4-pi-tap-hint` | 可點擊輸入框容器 |
| `.b4-pi-modal-card` | 浮動數字鍵盤 modal |
| `.b4-hnp-card` | 困難差額彈窗數字輸入器 |
| `.b4-hfh-card` / `.b4-hfh-formula` / `.b4-hfh-ans` | 算式提示彈窗 |
| `.b4-sl-summary-card` | 省錢清單 summary 卡片 |
| `.b4-dff-unknown` | 算式中的 ？？？ 格 |

### 21.4 新增方法摘要（Round 46）

| 方法 | 說明 |
|------|------|
| `_setupNormalModeCoins` | 普通模式點幣，兩側完成 → 顯示輸入框 |
| `_showPriceInputSection` | 點擊式輸入框 + 浮動 modal 數字鍵盤 |
| `_revealNormalDiffOptions` | 普通模式差額選項揭露 |
| `_showHardDiffNumpadModal` | 困難差額浮動彈窗輸入器 |
| `_showHardDiffFormulaHint` | 困難算式提示彈窗（含語音） |

---

*本節由 Claude Sonnet 4.6 協助撰寫，日期：2026-04-10*
