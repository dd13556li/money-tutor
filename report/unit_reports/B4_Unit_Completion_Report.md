...
| 項目 | 搜尋關鍵字 |
|------|------------|
| B4 錯誤語音統一 + 提示按鈕吉祥物（2026-04-03）| `不對喔，請再比較看看`、`不對喔，差額是` |
| B4 三商店 3次錯誤自動提示（2026-04-04）| `tripleAutoHint`、`tripleHintSpeech` |
| B4 差額漸進提示（2026-04-04）| `diffErrorCount`、`revealAnswer`、`revealTriple`、`兩者差多少元` |
| B4 自訂價格功能（2026-04-04）| `customItemsEnabled`、`_applyCustomPrices`、`b4-cpp-apply-btn` |
| B4 商品介紹彈窗 afterClose 模式（2026-04-05）| `afterClose`、`b4-item-intro-modal`、`_startMemoryCountdown` |
| B4 紙鈔統一尺寸 100×48.91px（2026-04-10）| `48.91px` |
| B4 簡單模式改逐枚點幣後自動比較（2026-04-10）| `_setupEasyModeCoins`、`b4-easy-coin`、`_handleEasyBothSidesDone` |
| B4 普通模式差額頁版面重構（2026-04-10）| `b4-diff-normal-card`、`b4-diff-options-card` |
| B4 省錢清單加標題列＋卡片版面（2026-04-10）| `b4-sl-page`、`b4-sl-card2`、`b4-sl-summary-bar` |
| B4 普通模式第一頁點幣→輸入較便宜價格（2026-04-10）| `_setupNormalModeCoins`、`_showPriceInputSection`、`b4-pi-card` |
| B4 困難模式第一頁語音後輸入價格（2026-04-10）| `diff === 'hard'` in afterClose |
| B4 商品彈窗簡化＋頁面語音修正（2026-04-10）| `b4-intro-card`、`請問哪一個商店賣的比較便宜` |
| B4 點幣最後一枚語音完整播放（2026-04-10）| `isLast`、`done = true` |
| B4 困難模式第一頁價格顯示？？？（2026-04-10）| `b4-price-unknown` |
| B4 普通模式第二頁選項？？？＋提示揭露（2026-04-10）| `b4-diff-opt-masked`、`_revealNormalDiffOptions` |
| B4 困難模式第二頁點？？？開彈窗＋提示算式（2026-04-10）| `_showHardDiffNumpadModal`、`_showHardDiffFormulaHint`、`b4-hnp-card`、`b4-hfh-card` |
| B4 普通/困難第一頁點輸入框開彈窗（2026-04-10）| `openNumpad`、`b4-pi-modal`、`b4-pi-tap-hint` |
| B4 各頁面語音內容修正（2026-04-10）| `請選擇正確的答案` |
| B4 省錢清單 summary 卡片化（2026-04-10）| `b4-sl-summary-card`、`b4-sl-sc-trophy` |
| B4 ref card 置中（2026-04-10）| `b4-diff-ref-cheap` |
| B4 第二頁簡單/困難版面重構（2026-04-10）| `hardFormulaHTML`、`b4-dff-unknown`、`hideResult` |
| B4 作業單難度分級（2026-04-25）| `this._items[difficulty]`、`keyPrefix`、`adjustCountButton` in b4-worksheet.js |
