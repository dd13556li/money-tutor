...
> **更新日期**：2026-04-03（互動豐富化：①付款頁提示按鈕（簡單/普通模式）新增吉祥物圖示外包裹；②找零題目錯誤語音統一含方向：「不對喔，找零算太多了/太少了，請再試一次」（`b6ChangeDir` 變數；取代原「再想想看，找回多少元？」）；搜尋關鍵字：`b6ChangeDir`、`不對喔，找零算`）
> **更新日期**：2026-04-03（第二輪豐富化：提示語音格式改為「可以用N個X元，M個Y元」（永遠含數量，分隔符「，」，取代原「建議付面額×數量，再加…」）；搜尋關鍵字：`speechParts`、`可以用`）
