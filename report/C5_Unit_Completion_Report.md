...
> **日期**：2026-02-09
> **時間**：下午
> **更新日期**：2026-04-07（反復測試錯誤語音精簡）、2026-03-30（題目框新增🔊朗讀按鈕）、2026-03-27（錢不夠差額金錢圖示 + 面額預設按鈕 + 預設連動 + 困難模式提示語音）、2026-03-14（輔助點擊彈窗偵測修復 + 設定頁說明更新）
...
- 錯誤答案語音分支依 `mode` 判斷：`mode === 'single'` 維持原有完整語音（含 `endingText`）；`mode !== 'single'`（反復測試）改為 `message = '不對喔，請再試一次'`，不含方向提示或進題語音

### 關鍵搜尋詞

`不對喔，請再試一次`、`mode === 'single'`（C5 錯誤分支）
