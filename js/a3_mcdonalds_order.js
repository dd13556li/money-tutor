...
document.addEventListener('DOMContentLoaded', () => {
    const McDonald = {
        ...
        showPaymentHint() {
            ...
            // 困難模式：按提示後永久解鎖已付金額顯示
            // 若退回前有金額，先顯示該金額3秒，之後切換為即時顯示實際已付金額
            const _revealPaidAmount = () => {
                if (difficulty === 'hard') {
                    this.state.gameState.paidAmountRevealed = true; // 永久解鎖，後續拖曳即時更新
                    if (capturedPaidAmount > 0) {
                        // 先顯示退回前的金額3秒
                        this._updatePaidAmountDisplay(capturedPaidAmount);
                        this.TimerManager.clearByCategory('paidReveal');
                        this.TimerManager.setTimeout(() => {
                            this._updatePaidAmountDisplay(); // 切換為實際即時金額（此時為0）
                        }, 3000, 'paidReveal');
                    }
                    // capturedPaidAmount === 0：已付金額立即顯示 NT$ 0
                }
            };
            ...
            // 退回後 DOM 已同步重建，直接呼叫（不用計時器，用戶點擊比計時器快）
            this.state.gameState.walletHintMoney = optimalMoney;
            this.showWalletHintWithTicks(optimalMoney);
            _revealPaidAmount();

            // 播放語音提示
            this.speech.speakText(this.generateHintSpeech(optimalMoney));

            this.Debug.log('flow', '[A3-McDonald] 顯示付款提示', optimalMoney.map(m => m.value));
        },
