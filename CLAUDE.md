...
| B6 找零階段完全重設計（2026-04-17）| B6 | `_b6P2ShowChangeReturn` 完整重建頁面（`b6c-change-page`）含：①標題卡（找零N元＋進度條）；②我的錢包區（drop zone）；③確認找零按鈕＋提示鈕（吉祥物）；④面額托盤（可重複拖曳，一個面額一張卡，依找零金額動態決定）；新函數 `_b6P2SetupChangeInteraction`、`_b6P2UpdateChangeDisplay`、`_b6P2RenderWalletCoins`、`_b6P2ConfirmChange`、`_b6P2ShowChangeGhostSlots`、`_b6P2ShowChangeHintModal`；面額選擇邏輯：貪婪解用到的面額＋最小面額下一個小的（干擾）＋第一個大於找零的（干擾）；Ghost slot 模式（`g.changeGhostMode`）：普通模式3次錯誤自動觸發，清空錢包後顯示淡化正確面額圖示，只接受對應面額；困難模式提示鈕開彈窗（`_b6P2ShowChangeHintModal`）；CSS 新增 `b6c-` 前綴規則；搜尋 `b6c-change-page`、`b6c-denom-card`、`b6c-wallet-zone`、`b6c-confirm-btn`、`_b6P2ConfirmChange`、`changeGhostMode`、`changeGreedySolution` |
| B5 第一頁提示系統同步 B6（2026-04-18）| B5 | ①任務卡：移除 `progress-bar-wrap`，新增 `b5-task-card`（紫色漸層）含標題+預算+吉祥物+`b5-p1-hint-btn`；②hint 狀態：`g.p1HintMode`/`g.p1HintItems`/`g.p1ErrorCount`；③簡單模式：關卡介紹關閉後自動 `_b5P1ActivateHintMode()`，一次高亮1個商品（`b5-hint-here`），不可取消/不可選非提示商品，全選後高亮確認按鈕；④普通模式：選商品時立即偵測超預算→錯誤toast+計數，3次→自動 `_b5P1ShowHint()`；⑤困難模式：提示鈕→ `_showHardModeHintModal()`；⑥`_b5P1ShowWrongTip` toast；⑦AssistClick 加 `p1HintMode` 分支（類別切換 + 提示商品順序）；搜尋 `b5-task-card`、`b5-p1-hint-btn`、`b5-hint-here`、`_b5P1ShowHint`、`_b5P1ActivateHintMode`、`p1HintMode` |
| B5 轉場歡迎畫面（2026-04-18）| B5 | `startGame()` 改呼叫 `showWelcomeScreen()`；2頁式（B6 pattern）：Page1 紫色漸層+派對主題icon+標題語音→500ms→Page2；Page2 預算+金幣圖示+商品emoji+「開始挑選！」按鈕；點開始→`g._skipIntroModal=true`→`renderRound()`（跳過關卡介紹彈窗）；`renderRound()` 加 `_skipIntroModal` 分支；AssistClick `buildQueue()` 加 `b5-wc2-start-btn` 偵測；搜尋 `showWelcomeScreen`、`b5-wc2-start-btn`、`_skipIntroModal` |
| B4 作業單難度分級（2026-04-25）| B4 作業單 | `_items` 拆成 `easy`(10項,max≤100)、`normal`(8項,59-280)、`hard`(6項,320-1580)三個子陣列；新增「水壺」「桌遊」(normal) 和「玩具組」「書包」(hard) 補足題數；`toolbarConfig.adjustCountButton` 加回（`🎯 難度` dropdown）；`generate()` 加 `difficulty` 參數；`usedKeys` 改用 `b4_${difficulty}_${idx}` 前綴防跨難度衝突；搜尋 `this._items[difficulty]`、`keyPrefix`、`adjustCountButton` in b4-worksheet.js |
| B5 找零階段同步 B6（2026-04-19）| B5 | 簡單模式：語音完成後自動顯示 ghost slots（`diff==='easy'` callback）；`_b5P2UpdateChangeTrayHints` 新增（ghost 模式下高亮托盤面額卡 `b6-product-here-hint`）；`_b5P2UpdateChangeDisplay` 改顯示錢包餘額+找回進度（`b5c-wallet-balance`/`b5c-placed-total`），移除進度條；`_b5P2RenderWalletCoins` 改 DOM diff + 簡單模式不顯示移除按鈕；困難模式：錯誤後保留錢包內容（`diff !== 'hard'` 守衛）；提示鈕：困難→揭露 `b5c-wallet-info`+彈窗，簡單/普通→ghost slots；拖曳/移除加 `diff !== 'easy'` 守衛；CSS 新增 `b5c-wallet-info`/`b6c-hidden`/`b6-product-here-hint` 至 `b5_party_budget.css`；搜尋 `_b5P2UpdateChangeTrayHints`、`b5c-wallet-info`、`b6c-hidden`、`找為` |
| B5 第二頁付款＋找零同步 B6（2026-04-17）| B5 | ①付款方式：簡單模式移除確認按鈕（all-filled 自動確認）；`_b5P2AddCoin` 播累加金額語音後 callback 觸發下一動作（簡單→自動確認；普通→「金額足夠，可以確認付款了」）；普通模式3次錯誤自動呼叫 `_showB5HardModeHintModal`；②找零階段：`_b5P2ShowChangeReturn` 完整重建頁面（`b5p2-*` 已有 CSS 類別 + `b5c-*` 新增 CSS 類別）含：標題卡（找零N元＋進度條）、我的錢包區（drop zone）、確認找零按鈕＋提示鈕（吉祥物）、面額托盤（可重複拖曳）；`_b5P2SetupChangeInteraction`、`_b5P2UpdateChangeDisplay`、`_b5P2RenderWalletCoins`、`_b5P2AddChangeCoin`、`_b5P2ConfirmChange`、`_b5P2ShowChangeGhostSlots`、`_b5P2ShowChangeHintModal`；Ghost slot 模式（`g.changeGhostMode`）：普通3次錯誤自動觸發，簡單/普通提示鈕觸發；困難提示鈕→彈窗；AssistClick 加 `b5c-wallet-zone` 偵測分支；CSS 新增 `b5c-` 前綴規則；搜尋 `b5c-denom-card`、`b5c-wallet-zone`、`b5c-confirm-btn`、`_b5P2SetupChangeInteraction`、`_b5P2ConfirmChange`、`changeGhostMode`、`changeGreedySolution` |
